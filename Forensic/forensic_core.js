(function (root) {
  ...
  function parseJpegMarkers(bytes) {
    ...
    if (info.trailing_bytes > 0)
      info.warnings.push("Bytes appended after JPEG EOI marker");
    ...
    while (off < bytes.length - 4) {
      ...
      if (len < 2 || off + len > bytes.length) {
        info.warnings.push(
          "Invalid JPEG segment length near offset " + (off - 2),
        );
        break;
      }
      ...
      if (marker >= 0xe0 && marker <= 0xef) {
        ...
        for (
          var i = payloadStart;
          i < Math.min(payloadEnd, payloadStart + 64);
          i++
        ) {
    ...
    if (info.quantization_tables === 0)
      info.warnings.push("No JPEG quantization tables found");
  ...
  function analyzeNoise(imageData, opts) {
    ...
    for (var ty = 0; ty < h; ty += tile) {
      for (var tx = 0; tx < w; tx += tile) {
        var vals = [];
        for (
          var yy = Math.max(1, ty);
          yy < Math.min(h - 1, ty + tile);
          yy += step
        ) {
          for (
            var xx = Math.max(1, tx);
            xx < Math.min(w - 1, tx + tile);
            xx += step
          ) {
            var ii = (yy * w + xx) * 4;
        ...
        if (vals.length > 0) {
          ...
          if (m > avg + sd * 0.9 || m > high) {
            tiles.push({
              x: tx,
              y: ty,
              w: Math.min(tile, w - tx),
              h: Math.min(tile, h - ty),
              score: m,
            });
          }
        }
    ...
    if (matches.length < 4) {
      ...
      for (var yy = 0; yy <= h - size; yy += step * 2) {
        for (var xx = 0; xx <= w - size; xx += step * 2) {
          blocks.push({
            x: xx,
            y: yy,
            desc: blockDescriptor(data, w, xx, yy, size),
          });
        }
      }
      for (var a = 0; a < blocks.length && matches.length < 40; a++) {
        for (var b = a + 1; b < blocks.length && matches.length < 40; b++) {
          var d = Math.sqrt(
            Math.pow(blocks[a].x - blocks[b].x, 2) +
              Math.pow(blocks[a].y - blocks[b].y, 2),
          );
          if (d >= size * 2) {
  ...
  function metadataSignals(bytes, imageData, fileName) {
    ...
    if (jpeg.is_jpeg) {
      ...
      if (jpeg.trailing_bytes > 0)
        signals.push("JPEG has appended bytes after the end marker");
    ...
    if (imageData) {
      var w = imageData.width || imageData.w,
        h = imageData.height || imageData.h;
      if (w * h > 0 && w * h < 4096)
        signals.push("Image is unusually small for forensic analysis");
    }
    return {
      jpeg: jpeg,
      signals: signals,
      suspicion: clamp(signals.length / 5, 0, 1),
    };
  }
  ...
  function combineFindings(parts) {
    ...
    if (parts.copy_move)
      score += (parts.copy_move.suspicion || 0) * weights.copy_move;
    if (parts.metadata)
      score += (parts.metadata.suspicion || 0) * weights.metadata;
  ...
  function buildSummary(parts) {
    ...
    if (parts.ela && parts.ela.suspicion > 0.45)
      signals.push("Localized compression inconsistency detected by ELA");
    if (parts.noise && parts.noise.suspicion > 0.45)
      signals.push("Noise residuals vary strongly between regions");
    if (parts.copy_move && parts.copy_move.match_count > 0)
      signals.push(
        "Possible copy-move duplicated regions: " + parts.copy_move.match_count,
      );
