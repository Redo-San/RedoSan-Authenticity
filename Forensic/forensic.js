...
function forensicBlobFromCanvas(canvas, mime, quality) {
  return new Promise(function(resolve) {
    canvas.toBlob(function(blob) { resolve(blob); }, mime || 'image/jpeg', quality == null ? 0.86 : quality);
  });
}

async function forensicLoadBlobImage(blob) {
  var file = new File([blob], 'forensic-recompressed.jpg', { type: blob.type || 'image/jpeg' });
  return await loadImage(file);
}

function forensicDiffImageData(a, b) {
  var w = a.width || a.w, h = a.height || a.h;
  ...
  var avg = scores.reduce(function(s, v) { return s + v; }, 0) / Math.max(1, scores.length);
  var hot = 0;
  for (var p = 0; p < scores.length; p++) if (scores[p] > avg * 2.2 && scores[p] > 8) hot++;
  ...
  return {
    ...
    suspicion: Math.max(0, Math.min(1, (avg / 24) + (hot / Math.max(1, scores.length)) * 3))
  };
}

function forensicNoiseHeatmap(imgData, noise) {
  var w = imgData.width || imgData.w, h = imgData.height || imgData.h;
  var out = new ImageData(w, h);
  for (var i = 0; i < out.data.length; i += 4) {
    out.data[i] = 18; out.data[i + 1] = 22; out.data[i + 2] = 30; out.data[i + 3] = 255;
  }
...
async function analyzeForensics(file) {
  ...
  if (img.w > FORENSIC_MAX_DIMENSION || img.h > FORENSIC_MAX_DIMENSION) {
    throw new Error('Image dimensions (' + img.w + 'x' + img.h + ') exceed maximum allowed (' + FORENSIC_MAX_DIMENSION + 'px). Please use a smaller image.');
  }
  var bytes = new Uint8Array(await file.arrayBuffer());
  var canvas = forensicCanvasFromImageData(img.imgData);
  var recompressedBlob = await forensicBlobFromCanvas(canvas, 'image/jpeg', 0.86);
  ...
  var metadata = RedoSanForensicCore.metadataSignals(bytes, img.imgData, file.name);
  var combined = RedoSanForensicCore.combineFindings({ ela: ela, noise: noise, copy_move: copyMove, metadata: metadata });
  var signals = RedoSanForensicCore.buildSummary({ ela: ela, noise: noise, copy_move: copyMove, metadata: metadata });
  ...
  return {
    ...
    signals: signals,
    ela: { mean_difference: ela.mean_difference, max_difference: ela.max_difference, hot_pixel_ratio: ela.hot_pixel_ratio, suspicion: Number(ela.suspicion.toFixed(3)) },
    noise: noise,
    copy_move: copyMove,
    metadata: metadata,
    _visuals: { source: img.imgData, ela: ela.imageData, noise: forensicNoiseHeatmap(img.imgData, noise) }
  };
}

function forensicRiskBadge(level, score) {
  var cls = level === 'high' ? 'danger' : (level === 'medium' ? 'warn' : 'ok');
  var color = cls === 'danger' ? 'var(--danger)' : (cls === 'warn' ? '#f6a623' : 'var(--success)');
  return '<span style="display:inline-block;padding:6px 10px;border-radius:6px;background:' + color + '22;color:' + color + ';font-weight:700">' +
    escHtml(level.toUpperCase()) + ' · ' + score + '/100</span>';
}
...
function renderForensicResult(result) {
  ...
  var html = '<div class="result-success">' +
    '<strong>Forensic Risk</strong><br>' + forensicRiskBadge(result.risk_level, result.risk_score) +
    '</div>';
  ...
  html += '<tr><td>ELA mean</td><td>' + result.ela.mean_difference + '</td></tr>';
  html += '<tr><td>Noise residual</td><td>' + result.noise.mean_residual + ' ± ' + result.noise.stddev_residual + '</td></tr>';
  html += '<tr><td>Copy-move matches</td><td>' + result.copy_move.match_count + '</td></tr>';
  html += '<tr><td>JPEG markers</td><td>' + (result.metadata.jpeg && result.metadata.jpeg.is_jpeg ? 'JPEG, APP: ' + escHtml(result.metadata.jpeg.app_segments.join(', ') || 'none') : 'Not JPEG') + '</td></tr>';
  html += '</table>';
  html += '<div style="margin-top:12px"><strong>Signals</strong><ul style="margin:8px 0 0 18px">';
  for (var i = 0; i < result.signals.length; i++) html += '<li>' + escHtml(result.signals[i]) + '</li>';
  html += '</ul></div>';
  output.innerHTML = html;

  forensicRenderCanvas('forensic-ela-map', result._visuals.ela, 'ELA heatmap');
  forensicRenderCanvas('forensic-noise-map', result._visuals.noise, 'Noise inconsistency map');
  var cmWrap = document.getElementById('forensic-copy-map');
  cmWrap.innerHTML = '<div style="font-weight:700;margin-bottom:8px">Copy-move regions</div>';
  ...
  var blob = new Blob([JSON.stringify(publicResult, null, 2)], { type: 'application/json' });
  var url = URL.createObjectURL(blob);
  if (window._forensicLastUrl) URL.revokeObjectURL(window._forensicLastUrl);
  window._forensicLastUrl = url;
  dl.innerHTML = '<a href="' + url + '" download="' + escHtml(result.file.name) + '.forensic.json" class="btn">Download JSON Report</a>';
}
...
async function handleForensicAnalyze() {
  ...
  resultDiv.style.display = 'block';
  output.textContent = 'Analyzing image for compression, noise, duplicated regions, and metadata inconsistencies...';
  dl.innerHTML = '';
  ['forensic-ela-map','forensic-noise-map','forensic-copy-map'].forEach(function(id) {
  ...
  try {
    ...
    output.innerHTML = '<div class="result-error">Error: ' + escHtml(e.message) + '</div>';
  } finally {
    btn.disabled = false;
