...
async function readPdfText(raw) {
  ...
  for (var objId in objMap) {
    ...
    var dec2;
    try { dec2 = await inflatePdfData(stringToBytes(raw2)); } catch(e) { continue; }
...
async function main() {
  try {
    ...
    try {
      ...
      console.log('File:', si.file_name, '(' + si.width + 'x' + si.height + ', ' + si.format + ')');
    ...
    // ── Capacity ──
    var coverText = pdfText || 'Fallback text for watermark testing purposes only.';
    console.log('\n=== Capacity Estimates ===');
    ...
    for (var id in DOCW_ALGOS) {
      ...
      try {
        ...
        console.log('  Embedded length: ' + coverText.length + ' -> ' + embedded.length);
        console.time('  extract');
        var extracted = await docwExtract(embedded, id);
        console.timeEnd('  extract');
        console.log('  Extracted: "' + extracted + '"');
        console.log('  Result: ' + (extracted === testMsg ? '✓ PASS' : '✗ FAIL'));
      } catch (err) {
      ...
      for (var id in DOCW_ALGOS) {
        ...
        if (cap >= secretStr.length) {
          try {
            ...
            console.log('  Full secret: ' + (x1 === secretStr ? '✓ PASS' : '✗ FAIL'));
          } catch (err) {
            console.log('  Full secret error: ' + err.message);
          }
        } else {
          console.log('  (Cover too short for full ' + secretStr.length + '-byte secret)');
        }
        ...
        if (cap >= hashLabel.length) {
          try {
            ...
            console.log('  SHA-256 label: ' + (x2 === hashLabel ? '✓ PASS' : '✗ FAIL'));
          } catch (err) {
            console.log('  SHA-256 label error: ' + err.message);
          }
        } else {
          console.log('  (Cover too short for ' + hashLabel.length + '-byte SHA-256 label)');
        }
        ...
        if (cap >= hashVal.length) {
          try {
            ...
            console.log('  SHA-256 value: ' + (x3 === hashVal ? '✓ PASS' : '✗ FAIL'));
          } catch (err) {
            console.log('  SHA-256 value error: ' + err.message);
          }
        } else {
          console.log('  (Cover too short for ' + hashVal.length + '-byte SHA-256 value)');
        }
      }
    }
    ...
    var pdfBytes = new Uint8Array(fs.readFileSync('F:\\7b8ca631-026f-4a50-84fa-c08d52bc9ae1.pdf'));
    ...
    // 2. Rebuild PDF
    var rebuiltBytes = await buildWatermarkedPdfDoc(pdfBytes, pdfText, watermarkedText);
    console.log('Rebuilt PDF size:', rebuiltBytes.length);

    // Save rebuilt PDF for inspection
    fs.writeFileSync('F:\\RedoSan Authenticity\\rebuilt_test.pdf', Buffer.from(rebuiltBytes));
    ...
    console.log('Round-trip PASS:', roundtripResult === testMsgRT ? '✓ YES' : '✗ NO');
    ...
    for (var algoId of [2, 3]) {
        ...
        console.log('Algorithm ' + algoId + ' direct extract:', rt2 === testMsgRT ? '✓' : '✗');
    }
...
async function buildWatermarkedPdfDoc(originalBytes, originalText, watermarkedText) {
    ...
    while ((m = re.exec(src)) !== null) {
        ...
        var modified = cleanData, didReplace = false;

        // Try decompress via polyfilled _inflate (zlib)
        var dec = null;
        try { dec = await _inflate(rawBytes); } catch (e1) { dec = null; }
        ...
        if (dec) {
            ...
            if (newStr !== decStr || streamModified) {
                ...
                for (var nb = 0; nb < toCompress.length; nb++) nBytes[nb] = toCompress.charCodeAt(nb) & 0xFF;
