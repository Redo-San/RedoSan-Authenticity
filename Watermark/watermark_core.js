...
function rgbToYcbcr(imgData) {
  ...
  const Y = new Float64Array(w * h),
    Cb = new Float64Array(w * h),
    Cr = new Float64Array(w * h);
  for (let i = 0; i < w * h; i++) {
    const r = data[i * 4],
      g = data[i * 4 + 1],
      b = data[i * 4 + 2];
...
function ycbcrToImageData(Y, Cb, Cr, w, h) {
  ...
  for (let i = 0; i < w * h; i++) {
    ...
    imgData.data[i * 4 + 1] = Math.max(
      0,
      Math.min(255, y - 0.3441 * cb - 0.7141 * cr),
    );
...
function blockIter(w, h, bsize) {
  ...
  for (let y = 0; y < h - (h % bsize); y += bsize)
    for (let x = 0; x < w - (w % bsize); x += bsize) blocks.push([x, y]);
  return blocks;
}
/**
 *
...
function getBlock8(arr, w, x, y) {
  ...
  for (let dy = 0; dy < 8; dy++)
    for (let dx = 0; dx < 8; dx++) b[dy][dx] = arr[(y + dy) * w + (x + dx)];
  return b;
}
/**
 *
...
function setBlock8(arr, w, x, y, block) {
  for (let dy = 0; dy < 8; dy++)
    for (let dx = 0; dx < 8; dx++) arr[(y + dy) * w + (x + dx)] = block[dy][dx];
}
  ...
  for (let y = 0; y < h && idx < payloadBits.length; y++) {
    for (let x = 0; x < w && idx < payloadBits.length; x++) {
      ...
      if (idx < payloadBits.length) {
        data[i] = (data[i] & ~1) | parseInt(payloadBits[idx], 2);
        idx++;
      }
      if (idx < payloadBits.length) {
        data[i + 1] = (data[i + 1] & ~1) | parseInt(payloadBits[idx], 2);
        idx++;
      }
      if (idx < payloadBits.length) {
        data[i + 2] = (data[i + 2] & ~1) | parseInt(payloadBits[idx], 2);
        idx++;
      }
    }
  }
...
function wm6_embed(imgData, payloadBits) {
  ...
  for (let y = 0; y < h && idx < payloadBits.length; y++) {
    for (let x = 0; x < w && idx < payloadBits.length; x++) {
      const i = (y * w + x) * 4;
      if (idx < payloadBits.length) {
        const b1 = parseInt(payloadBits[idx], 2),
          b2 =
            idx + 1 < payloadBits.length
              ? parseInt(payloadBits[idx + 1], 2)
              : 0;
        data[i] = (data[i] & ~3) | ((b1 << 1) | b2);
        idx += 2;
      }
      if (idx < payloadBits.length) {
        const b1 = parseInt(payloadBits[idx], 2),
