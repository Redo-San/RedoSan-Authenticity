...
var ASSISTANT_KB = [
  ...
  { id:'forensic', patterns:['forensic','tamper','ela','noise analysis','copy move','jpeg markers','image forgery','image tamper','تحليل جنائي','طب شرعي','تلاعب','تحليل الصور'],
    ...
    suggestions:{ en:['How does ELA work?','What is copy-move detection?','What tamper signals exist?'], ar:['كيف يعمل ELA؟','ما هو كشف النسخ واللصق؟','ما هي إشارات التلاعب؟']}},
...
function showInitialGreeting() {
  ...
  if (typeof REDOSAN_BOT_CHECK !== 'undefined' && REDOSAN_BOT_CHECK && REDOSAN_BOT_CHECK.isAutomated) {
    var botLang = getAssistantLang();
    addMessage(botLang === 'ar'
      ? '⚠️ **تم اكتشاف متصفح آلي.** تطبيق RedoSan Authenticity مخصص للمستخدمين البشريين فقط. يرجى تعطيل أدوات الأتمتة.'
      : '⚠️ **Automated browser detected.** RedoSan Authenticity is intended for human users only. Please disable automation tools.', 'bot');
    return;
  }
  var ctx = getCurrentContext();
  var initialMsg = ctx ? {
