...
async function upgradeOts(bytes) {
  ...
  for (var url of OTS_AGGREGATORS) {
    try {
      ...
      var to = setTimeout(function() { ac.abort(); }, 15_000);
...
function getOtsUpgradeCommand(fileName) {
  ...
  return [
    ...
    '#   $hash = (Get-Item "' + fileName.replaceAll('"', '`"') + '.ots").OpenRead();' + " $hash.Position = $hash.Length - 32;" + ' $bytes = [byte[]]::new(32);' + ' $hash.Read($bytes, 0, 32) | Out-Null;' + ' $hash.Close();' + ' Invoke-WebRequest -Uri https://a.pool.opentimestamps.org/digest -Method POST -ContentType "application/x-www-form-urlencoded" -Body $bytes -OutFile "' + fileName.replaceAll('"', '`"') + '.ots.upgraded"'
  ].join('\n');
}
...
async function handleOtsCreate() {
  ...
  } catch (error) { setText('ts-output', __('shared.error_prefix', 'Error: ') + error.message); }
...
async function handleOtsVerify() {
  ...
  try {
    ...
    var storedHex = [...parsed.hash].map(function(b) { return b.toString(16).padStart(2, '0'); }).join('');
    var fileHex = [...fileHash].map(function(b) { return b.toString(16).padStart(2, '0'); }).join('');
