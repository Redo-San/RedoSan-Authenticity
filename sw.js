...
self.addEventListener('fetch', function(event) {
  ...
  if (url.origin === self.location.origin) {
    ...
    if (lower.endsWith('.js')) isBlocked = isBlocked || JS_WHITELIST.indexOf(path) === -1;
    // Unknown .css files not in CSS_WHITELIST
    if (lower.endsWith('.css')) isBlocked = isBlocked || CSS_WHITELIST.indexOf(path) === -1;
    // Unknown .yml/.yaml files not in YML_WHITELIST
    if (lower.endsWith('.yml') || lower.endsWith('.yaml')) isBlocked = isBlocked || YML_WHITELIST.indexOf(path) === -1;
    // Unknown .html files not in HTML_WHITELIST
    if (lower.endsWith('.html')) isBlocked = isBlocked || HTML_WHITELIST.indexOf(path) === -1;
    ...
    if (lower.endsWith('.js') || lower.endsWith('.css') || lower.endsWith('.html') ||
        lower.endsWith('.yml') || lower.endsWith('.yaml') || lower.endsWith('.wasm')) {
