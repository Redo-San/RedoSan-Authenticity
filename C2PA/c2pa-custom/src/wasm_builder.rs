...
impl WasmBuilder {
    ...
    /// Sign an asset using the provided SignerDefinition, format, and source bytes (Uint8Array).
    /// Unlike `sign()`, this avoids FileReaderSync and runs on the main thread.
    #[wasm_bindgen(js_name = signFromBytes)]
    pub async fn sign_from_bytes(
        &mut self,
        signer_definition: &SignerDefinition,
        format: &str,
        source: Vec<u8>,
