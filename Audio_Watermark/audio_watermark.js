...
function switchAwmTab(mode) {
  document
    .querySelectorAll(".tab-btn[data-awm-tab]")
    .forEach((b) => b.classList.remove("active"));
  document.getElementById("awm-embed").style.display =
    mode === "embed" ? "" : "none";
  document.getElementById("awm-extract").style.display =
    mode === "extract" ? "" : "none";
  document
    .querySelector('.tab-btn[data-awm-tab="' + mode + '"]')
...
function loadAwmFile(event) {
  ...
  reader.onload = function (e) {
    ...
    document.getElementById("awm-file-name").textContent =
      file.name + " (" + _awmSecretBytes.length + " bytes)";
    updateAwmCapacity();
  };
...
function updateAwmCapacity() {
  ...
  if (isLowCapacity) {
    ...
    msgBytes = txt
      ? new TextEncoder().encode(txt.substring(0, 1000))
      : new Uint8Array(0);
    _awmSecretBytes = msgBytes;
    document.getElementById("awm-text-info").textContent =
      msgBytes.length + " bytes / ~" + msgBytes.length * 8 + " bits";
  } else {
  ...
  var descs = {
    1: "LSB: ~1 bit/sample",
    2: "Phase Coding: ~1 bit/sample",
    3: "Echo Hiding: ~1 bit/" + AWM3_FRAME + " samples x3 reps",
    4: "DSSS: ~1 bit/" + (AWM4_FRAME >> 1) + " frames",
    5: "QIM: ~1 bit/sample",
    6: "DWT Haar: ~1 bit/1024 coefs",
    7: "Patchwork: ~1 bit/" + (AWM7_FRAME >> 1) + " frames x5 reps",
    8: "DCT: ~1 bit/" + (AWM8_FRAME >> 1) + " frames",
  };
  var mult = {
    1: 1,
    2: 1,
...
async function handleAwmEmbed() {
  ...
  try {
    ...
    if (payloadBits.length > maxBits)
      throw new Error(
        "Message too long. Need " +
          payloadBits.length +
          " bits, max " +
          maxBits +
          " for " +
          (names[type] || "this algorithm"),
      );
    var modified;
    ...
    if (type === 8) {
      ...
      modified = await aw8_embed_async(
        s16,
        payloadBits,
        info.sr,
        function (pct) {
    ...
    output.innerHTML =
      '<div class="result-success"><span class="result-icon">✅</span><strong>Watermark embedded successfully!</strong><br>Algorithm: ' +
      names[type] +
      "<br>File: " +
      escapeHtml(audioFile.name) +
      "<br>Sample Rate: " +
      info.sr +
      " Hz<br>Channels: " +
      info.ch +
    ...
    downloadDiv.innerHTML =
      ...
      escapeHtml(audioFile.name).replace(/\.[^.]+$/, ".wav") +
      '" class="btn">⬇ Download Watermarked WAV</a>';
    setResult("awmResult", {
      blob: blob,
      originalName: audioFile.name,
      type: type,
      algorithm: names[type],
    });
    resultDiv.style.display = "";
  } catch (error) {
    prog.style.display = "none";
    output.innerHTML =
      '<div class="result-error"><span class="result-icon">❌</span>' +
...
async function handleAwmExtract() {
  ...
  if (!input || !input.files || !input.files[0])
    return alert("Please select a watermarked audio file");
  if (typeof validateFileInput === "function" && !validateFileInput(input))
    return;
  var audioFile = input.files[0];
  ...
  try {
    ...
    var detectedAlgo = "";
    if (type === 0) {
      var found = await awmMultiDetect(
        info,
        key,
        spinner,
        prog,
      ...
      spinner.style.display = "none";
      prog.style.display = "flex";
      bitsStr = await aw8_extract_async(
        info.samples,
        info.sr,
        info.samples.length,
        function (pct) {
    ...
    if (type !== 0) {
      ...
      if (result === "bad-password") {
        output.innerHTML =
          '<div class="result-error"><span class="result-icon">❌</span>Wrong password or no watermark found</div>';
      } else if (result) {
        ...
        var algoName =
          ...
          "Unknown";
        output.innerHTML =
          '<div class="result-success"><span class="result-icon">✅</span><strong>Watermark extracted!</strong><br>Algorithm: ' +
          algoName +
          '<br><br><strong>Hidden Message:</strong><br><pre class="awm-pre">' +
          escapeHtml(decoded) +
          "</pre></div>";
      } else {
        output.innerHTML =
          '<div class="result-error"><span class="result-icon">❌</span>No watermark data found. Try a different algorithm.</div>';
      }
    }
    downloadDiv.innerHTML = "";
    resultDiv.style.display = "";
...
async function awmMultiDetect(info, key, spinner, prog, progFill, progText) {
  ...
  function makeAlgos(src) {
    return [
      ...
      },
      {
        ...
        name: "Echo Hiding",
        fn: function () {
          return aw3_extract(src, info.sr);
        },
      },
      {
        id: 4,
  ...
  for (var ci = 0; ci < channels.length; ci++) {
    ...
    for (var i = 0; i < algos.length; i++) {
      ...
      progText.textContent =
        "✅ " + a.name + " (" + ch.label + "): watermark found!";
      await new Promise(function (r) {
        setTimeout(r, 200);
      });
    }
  ...
  if (found.length === 0) {
    output.innerHTML =
      '<div class="result-error"><span class="result-icon">❌</span>No watermark detected with any algorithm. Try a different password or file.</div>';
    downloadDiv.innerHTML = "";
    resultDiv.style.display = "";
    return false;
  }
  ...
  for (var j = 0; j < found.length; j++) {
    ...
    var icon = j === 0 ? "✅" : "🔷";
    html +=
      '<div class="result-success" style="margin-bottom:12px"><span class="result-icon">' +
      icon +
      "</span>" +
      "<strong>Watermark #" +
      (j + 1) +
      ...
      f.channel +
      ")<br><br>" +
      '<strong>Hidden Message:</strong><br><pre class="awm-pre">' +
      escapeHtml(f.decoded) +
      "</pre></div>";
  }
...
async function awmDualExtract() {
  ...
  try {
    ...
    if (
      info.ch >= 2 &&
      info.raw &&
      info.raw.length >= info.samples.length * 2
    ) {
      rightSrc = new Int16Array(info.samples.length);
      for (var ri = 0; ri < info.samples.length; ri++)
        rightSrc[ri] = info.raw[ri * info.ch + 1];
    } else {
      var frameSize =
        fpAlgo && fpAlgo > 0
          ? fpAlgo === 1 || fpAlgo === 5
      ...
      if (splitPoint < frameSize)
        splitPoint = Math.floor(info.samples.length / 2);
    ...
    function tryExtract(src, algo, label) {
      if (algo === 0) {
        ...
        var candidates =
          label === "DID Identity" ? [2, 6, 8] : [1, 2, 3, 5, 6, 7, 8];
        return tryExtractAuto(src, key, info.sr, candidates);
      }
      return tryExtractSingle(
        src,
        algo,
        key,
        info.sr,
        algoNames[algo] || "Algo " + algo,
      );
    }
    ...
    for (var rj = 0; rj < results.length; rj++) {
      ...
      html +=
        res2 && res2.decoded
          ? '<div class="result-success" style="margin-bottom:12px"><span class="result-icon">✅</span>' +
            "<strong>" +
            results[rj].label +
            "</strong><br>Algorithm: " +
            res2.algorithm +
            "<br>" +
            '<strong>Hidden Message:</strong><br><pre class="awm-pre">' +
            escapeHtml(res2.decoded) +
            "</pre></div>"
          : '<div class="result-error" style="margin-bottom:12px"><span class="result-icon">❌</span>' +
            "<strong>" +
            results[rj].label +
            "</strong><br>" +
            (res2 ? res2.error : "not found") +
            "</div>";
    }
    output.innerHTML =
      html || '<div class="result-error">❌ No watermarks found.</div>';
    downloadDiv.innerHTML = "";
    resultDiv.style.display = "";
  } catch (error) {
    prog.style.display = "none";
...
async function awmSelfTest() {
  ...
  try {
    ...
    for (var i = 0; i < len; i++) {
      v.setInt16(
        44 + i * 2,
        Math.round(8000 * Math.sin((2 * Math.PI * 440 * i) / sr)),
        true,
      );
    }
    ...
    var algos = [
      {
        ...
        name: "LSB",
        embed: function () {
          return aw1_embed(new Int16Array(s16), payload);
        },
        extract: function (m) {
          return aw1_extract(m, m.length);
        },
      },
      {
        id: 2,
        name: "FFT-QIM",
        embed: function () {
          return aw2_embed(new Int16Array(s16), payload, sr);
        },
        extract: function (m) {
          return aw2_extract(m, sr, m.length);
        },
      },
      {
        id: 5,
        name: "QIM",
        embed: function () {
          return aw5_embed(new Int16Array(s16), payload, sr);
        },
        extract: function (m) {
          return aw5_extract(m, sr, m.length);
        },
      },
      {
        id: 6,
        name: "DWT",
        embed: function () {
          return aw6_embed(new Int16Array(s16), payload, sr);
        },
        extract: function (m) {
          return aw6_extract(m, sr, m.length);
        },
      },
      {
        id: 8,
        ...
        embed: async function () {
          return await aw8_embed_async(new Int16Array(s16), payload, sr);
        },
        extract: async function (m) {
          return await aw8_extract_async(m, sr, m.length);
        },
      },
    ];
      ...
      try {
        var maxB =
          algo.id === 1 || algo.id === 5
            ...
            : Math.floor(
                s16.length /
                  (algo.id === 8 ? 1024 : algo.id === 6 ? 1024 : 2048),
              );
        if (payload.length > maxB) {
          results.push(algo.name + ": SKIP");
          continue;
        }
    ...
    var fpBits2 = awFormatPayload(
      new TextEncoder().encode("DUAL_FP_TEST"),
      key,
    );
    var tsBits2 = awFormatPayload(
      new TextEncoder().encode("DUAL_TS_TEST"),
      key,
    );
    if (fpBits2.length <= fpMaxB && tsBits2.length <= tsMaxB) {
      try {
        ...
        for (var si = 0; si < stereoLen; si++)
          rightCh[si] = stereoInfo.raw[si * 2 + 1];

        var fpBitsOut = await aw8_extract_async(
          stereoInfo.samples,
          sr,
          stereoLen,
        );
        var fpR =
          fpBitsOut && fpBitsOut.length >= 32
            ? awExtractPayload(fpBitsOut, key)
            : null;
        var fpOk =
          fpR &&
          fpR !== "bad-password" &&
          new TextDecoder().decode(fpR) === "DUAL_FP_TEST";

        var tsBitsOut = await aw6_extract(rightCh, sr, stereoLen);
        var tsR =
          tsBitsOut && tsBitsOut.length >= 32
            ? awExtractPayload(tsBitsOut, key)
            : null;
        var tsOk =
          tsR &&
          tsR !== "bad-password" &&
          new TextDecoder().decode(tsR) === "DUAL_TS_TEST";

        results.push(
          "Dual (DCT left + DWT right): " + (fpOk && tsOk ? "✅" : "❌"),
        );
        if (!fpOk)
          results.push(
            "  FP: " +
              (fpR ? new TextDecoder().decode(fpR).substring(0, 20) : "null"),
          );
        if (!tsOk)
          results.push(
            "  TS: " +
              (tsR ? new TextDecoder().decode(tsR).substring(0, 20) : "null"),
          );
      } catch (error) {
    ...
    spinner.style.display = "none";
    var html =
      '<div class="result-success"><span class="result-icon">🔬</span><strong>Self-Test Results</strong><br><br>' +
      results.join("<br>") +
      "<br><br><em>All ✅ means the watermark code works in this browser.</em></div>";
    output.innerHTML = html;
    downloadDiv.innerHTML = "";
    resultDiv.style.display = "";
  } catch (error) {
    spinner.style.display = "none";
    output.innerHTML =
      '<div class="result-error"><span class="result-icon">❌</span>Self-test error: ' +
      escapeHtml(error.message) +
      "</div>";
    downloadDiv.innerHTML = "";
    resultDiv.style.display = "";
