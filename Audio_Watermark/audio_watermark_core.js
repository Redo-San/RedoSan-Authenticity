...
function awReadWav(buf) {
  ...
  return {
    ...
    ch: fmt.ch,
    bps: fmt.bps,
    raw: new Int16Array(totalSamples),
    rawOff: dataOff,
  };
}
/**
...
function awWriteWav(mono, sr, ch, rawData, bps) {
  ...
  const frames = isDual
    ? mono[0].length
    : Math.max(
        rawData ? Math.floor(rawData.length / ch) : mono.length,
        mono.length,
      );
  ...
  for (let i = 0; i < frames; i++) {
    for (let c = 0; c < ch; c++) {
      ...
      v.setInt16(
        44 + (i * ch + c) * 2,
        Math.max(-32_768, Math.min(32_767, val || 0)),
        true,
      );
    }
  }
...
function awReadRightChannel(buf) {
  ...
  for (let i = 0; i < monoLen; i++)
    r[i] = v.getInt16(dataOff + (i * fmt.ch + 1) * 2, true);
  return r;
}
/* c8 ignore start */
/**
...
function awExtractPayload(bitsStr, key) {
  ...
  if (!dlen || dlen < 2 || dlen > 100_000 || bitsStr.length < 32 + dlen * 8)
    return null;
...
function aw3_embed(s16, bitsStr, sr) {
  ...
  const LO = Math.floor(F * 0.08),
    HI = Math.floor(F * 0.25);
  const step = Math.max(
    1,
    Math.floor((HI - LO - CHIPS) / Math.max(1, totalFrames)),
  );
  for (let f = 0; f < effectiveBits; f++) {
    const off = f * F;
    ...
    for (let i = 0; i < F; i++)
      s16[off + i] = Math.max(
        -32_768,
        Math.min(32_767, Math.round(reconstructed[i])),
      );
  }
  return s16;
}
...
function aw4_embed(s16, bitsStr, sr) {
  ...
  const strength = Math.max(
    30,
    Math.min(1200, Math.round((sr || 44_100) / 60)),
  );
  const step = Math.max(
    1,
    Math.floor((hi - lo - cap) / Math.max(1, totalFrames)),
  );
  for (let f = 0; f < effectiveBits; f++) {
    const off = f * F;
    const re = new Float64Array(F),
    ...
    for (let c = 0; c < cap; c++) {
      ...
      const newMag = Math.max(
        mag * 0.3,
        Math.min(mag * 3, mag + (bit === "1" ? 1 : -1) * PN[c] * strength),
      );
...
function aw4_extract(s16, sr, numBits) {
  ...
  for (let f = 0; f < maxBits; f++) {
    ...
    for (let c = 0; c < cap; c++) {
      const bin = chipStart + c;
      sumLog += Math.log(1 + Math.hypot(re[bin], im[bin]));
    }
...
function awDct(signal) {
  ...
  for (let k = 0; k < N; k++) {
    ...
    X[k] =
      (re[k] * Math.cos(theta) - im[k] * Math.sin(theta)) *
      (k === 0 ? scale0 : scale);
  }
  return X;
}
...
function aw7_embed(s16, bitsStr, sr) {
  ...
  const effectiveBits = Math.min(bitsStr.length, totalFrames);
  const LO = Math.floor(F * 0.1),
    HI = Math.floor(F * 0.35);
  const usable = HI - LO;
  const step = Math.max(
    1,
    Math.floor((usable - CHIPS) / Math.max(1, totalFrames)),
  );
  for (let f = 0; f < effectiveBits; f++) {
    const off = f * F;
    const frame = new Float64Array(F);
