...
async function test() {
    ...
    const tests = [
        { id: 2, name: 'Phase Coding', msg: 'Hello World! This is a test of phase coding algorithm with repetition.', embed: (s,p) => aw2_embed(s,p,sr), extract: (s,r) => aw2_extract(s,sr,r), maxBits: (l) => aw2_maxBits(l,sr) },
        ...
        { id: 8, name: 'DCT-based', msg: 'Hello World! DCT test of the fast version.', embed: (s,p) => aw8_embed(s,p,400), extract: (s,r) => aw8_extract(s,r,400), maxBits: (l) => aw8_maxBits(l,sr) }
    ];
    ...
    for (const t of tests) {
        ...
        let payload;
        if (fullPayload.length <= maxB) { payload = fullPayload; }
        else { payload = fullPayload.substring(0, maxB); console.log(`  [${t.name}] Truncating payload: ${fullPayload.length} → ${maxB}`); }
        ...
        try {
            ...
            const extractTime = Date.now() - start2;
            ...
            if (!result) status = '❌ No watermark';
                ...
                const expectedPayload = fullPayload.length > maxB ? fullPayload.substring(0, maxB) : fullPayload;
                const match = fullPayload.length <= maxB ? (decoded === t.msg) : (decoded.length > 0);
                status = match ? '✅' : '⚠️';
                if (decoded !== t.msg) {
                ...
                status += ` decoded:${decoded.length} bytes`;
            }

            console.log(`\n${t.name}:`);
            console.log(`  Payload: ${payload.length} bits (msg: "${t.msg}")`);
            console.log(`  Embed: ${(embedTime/1000).toFixed(2)}s | Extract: ${(extractTime/1000).toFixed(2)}s`);
            console.log(`  SNR: ${snr < -50 ? '-∞' : snr.toFixed(1)} dB`);
            console.log(`  ${status}`);
        } catch (e) {
