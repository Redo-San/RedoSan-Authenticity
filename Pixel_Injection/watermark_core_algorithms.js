...
WatermarkCore.prototype.vine = function (
  imageData,
  message,
  password = null,
  options = {},
      ...
      for (let c = 0; c < 3; c++) {
        data[pi + c] = Math.max(
          ...
          Math.min(
            255,
            data[pi + c] +
              adversarialPattern[(y * width + x) % adversarialPattern.length] *
...
WatermarkCore.prototype.pixelSeal = function (
  imageData,
  message,
  password = null,
  options = {},
      ...
      for (let c = 0; c < 3; c++) {
        data[pi + c] = Math.max(
          ...
          Math.min(
            255,
            data[pi + c] +
              watermarkPattern[(y * width + x) % watermarkPattern.length] *
                strength *
                jndMask[y * width + x],
          ),
        );
...
WatermarkCore.prototype.nullguard = function (
  imageData,
  message,
  password = null,
  options = {},
...
WatermarkCore.prototype.shallowDiffuse = function (
  imageData,
  message,
  password = null,
  options = {},
  ...
  const watermarked = this.dct(imageData, message, password, options);
  const data = new Uint8ClampedArray(watermarked.data);
  const diffusionPattern = this.generateDiffusionPattern(
    width,
    height,
    password,
  );
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (Math.abs(pv) > 0.3) {
        for (let c = 0; c < 3; c++) {
          data[pi + c] = Math.max(
            0,
            Math.min(255, data[pi + c] + Math.round(pv * 3)),
          );
        }
      }
    }
...
WatermarkCore.prototype.imagewmark = function (
  imageData,
  message,
  password = null,
  options = {},
...
WatermarkCore.prototype.metaSeal = function (
  imageData,
  message,
  password = null,
  options = {},
  ...
  switch (mediaType.toLowerCase()) {
    ...
    default: {
      return this.adaptiveDCT(imageData, message, null, {
        strength: 0.1,
...
WatermarkCore.prototype.stardustmark = function (
  imageData,
  message,
  password = null,
  options = {},
...
WatermarkCore.prototype.invisimark = function (
  imageData,
  message,
  password = null,
  options = {},
  ...
  return this.pixelSeal(imageData, message, null, {
    strength: adaptedStrength,
  });
};

// 14. ElevenLikes industrial implementation
WatermarkCore.prototype.elevenlikes = function (
  imageData,
  message,
  password = null,
  options = {},
...
WatermarkCore.prototype.diffusionBased = function (
  imageData,
  message,
  password = null,
  options = {},
      ...
      for (let c = 0; c < 3; c++) {
        data[pi + c] = Math.max(
          ...
          Math.min(
            255,
            data[pi + c] +
              diffusionPattern[(y * width + x) % diffusionPattern.length] * 2,
          ),
        );
      }
    }
...
WatermarkCore.prototype.statisticalDetection = function (watermarkedImageData) {
  const characteristics =
    this.analyzeStatisticalCharacteristics(watermarkedImageData);
...
WatermarkCore.prototype.robustnessTesting = function (
  originalImage,
  watermarkedImage,
) {
  const tests = [
...
WatermarkCore.prototype.calculateAdvancedJNDMask = function (
  data,
  width,
  height,
) {
...
WatermarkCore.prototype.analyzeAICharacteristics = function (
  imageData,
  aiModel,
) {
  return {
...
// Adaptive DCT
WatermarkCore.prototype.adaptiveDCT = function (
  imageData,
  message,
  strength,
  adaptiveStrength,
...
WatermarkCore.prototype.analyzeStatisticalCharacteristics = function (
  imageData,
) {
...
WatermarkCore.prototype.calculateSkewness = function (histogram) {
  const mean =
    histogram.reduce((sum, val, i) => sum + val * i, 0) /
    histogram.reduce((sum, val) => sum + val, 0);

  const variance =
    histogram.reduce((sum, val, i) => sum + Math.pow(i - mean, 2) * val, 0) /
    histogram.reduce((sum, val) => sum + val, 0);

  const skewness =
    histogram.reduce((sum, val, i) => sum + Math.pow(i - mean, 3) * val, 0) /
...
WatermarkCore.prototype.calculateMessageSimilarity = function (
  original,
  extracted,
) {
      ...
      switch (test.type) {
        case "compression": {
          recommendations.push(
            "Increase redundancy for better compression resistance",
          );
          break;
        }
        case "cropping": {
          recommendations.push(
            "Use spatial redundancy for cropping resistance",
          );
          break;
        }
        case "rotation": {
...
WatermarkCore.prototype.calculateSSIM = function (original, watermarked) {
  ...
  const varWatermarked = this.calculateVariance(
    watermarked.data,
    meanWatermarked,
  );
  ...
  const ssim =
    ...
    ((meanOriginal * meanOriginal + meanWatermarked * meanWatermarked + c1) *
      (varOriginal + varWatermarked + c2));
...
WatermarkCore.prototype.calculateCovariance = function (
  data1,
  data2,
  mean1,
  mean2,
...
WatermarkCore.prototype.calculatePerceptualMask = function (
  data,
  width,
  height,
) {
...
WatermarkCore.prototype.analyzeImageCharacteristics = function (
  data,
  width,
  height,
) {
...
WatermarkCore.prototype.generateDiffusionPattern = function (
  width,
  height,
  password = null,
) {
...
WatermarkCore.prototype.encodeMessageWithPassword = function (
  message,
  password,
) {
...
WatermarkCore.prototype.distributeMessageInSubBands = function (
  message,
  decomposition,
) {
...
WatermarkCore.prototype.selectOptimalCoefficients = function (
  dctBlock,
  strength,
) {
...
WatermarkCore.prototype.extractDCT = function (
...
WatermarkCore.prototype.extractDFT = function (
  watermarkedImageData,
  password = null,
  options = {},
) {
...
WatermarkCore.prototype.extractDWT = function (watermarkedImageData) {
  ...
  const bandLen =
    decomp._bandLen || Math.floor(width / 2) * Math.floor(height / 2) * 4;
  let bits = "";
...
WatermarkCore.prototype.extractLSB = function (watermarkedImageData) {
  ...
  if (isNaN(len) || len <= 0 || len > 100_000)
...
WatermarkCore.prototype.extractEnhancedLSB = function (watermarkedImageData) {
  ...
  if (bits.length < 32) return "No readable message found";
  const len =
    (parseInt(bits.substring(24, 32), 2) << 24) |
...
WatermarkCore.prototype.extractMultiChannelLSB = function (
  watermarkedImageData,
) {
  const data = watermarkedImageData.data;
  const width = watermarkedImageData.width;
...
WatermarkCore.prototype.extractRandomLSB = function (
  watermarkedImageData,
  password = null,
) {
  const data = watermarkedImageData.data;
...
WatermarkCore.prototype.getAdvancedJNDThreshold = function (
  brightness,
  contrast,
  texture,
) {
...
// Modulate phase
WatermarkCore.prototype.modulatePhase = function (
  spectrum,
  bit,
  phaseModulation,
) {
  ...
  return {
    real:
      spectrum.real * Math.cos(phaseModulation) -
      spectrum.imag * Math.sin(phaseModulation),
    imag:
      spectrum.real * Math.sin(phaseModulation) +
      spectrum.imag * Math.cos(phaseModulation),
  };
};

// Calculate local contrast
WatermarkCore.prototype.calculateLocalContrast = function (
  data,
  x,
  y,
  width,
...
WatermarkCore.prototype.calculateTextureComplexity = function (
  data,
  x,
  y,
  width,
...
WatermarkCore.prototype.calculateMultiFactorJND = function (
  brightness,
  contrast,
  texture,
) {
