...
class AdvancedWatermarkUI {
  ...
  createAdvancedOptions() {
    ...
    if (!optionsContainer) {
      ...
      const passwordField =
        document.getElementById("wm-password").parentElement;
  ...
  async handleAdvancedEmbed() {
    ...
    try {
      ...
      // Apply watermark
      this.watermarkedImage = await this.core.algorithms[this.currentAlgorithm](
        imageData,
        message,
        password,
        options,
      ...
      this.showMessage(
        `Watermark embedded successfully using ${this.currentAlgorithm}`,
        "success",
      );
    } catch (error) {
      this.showLoading(false);
  ...
  async handleAdvancedExtract() {
    ...
    const extractionStrength = parseFloat(
      document.getElementById("extraction-strength").value,
    );
    ...
    try {
      ...
      this.extractedMessage = await this.core.detection.blind_decoding(
        imageData,
        algorithm,
        options,
      );
      ...
      this.showMessage(
        `Watermark extracted successfully using ${algorithm}`,
        "success",
      );
    } catch (error) {
      this.showLoading(false);
  ...
  showExtractedMessage() {
    ...
    const downloadDiv = document.getElementById("wm-download");
    downloadDiv.innerHTML =
      '<button class="btn" id="wm-copy-btn" style="margin-top: 10px;">' +
  ...
  updateQualityDisplay() {
    ...
    document.getElementById("psnr-value").textContent =
      this.qualityMetrics.psnr.toFixed(2);
    document.getElementById("ssim-value").textContent =
      this.qualityMetrics.ssim.toFixed(4);
    document.getElementById("lpips-value").textContent =
      this.qualityMetrics.lpips.toFixed(4);
    document.getElementById("ber-value").textContent =
      this.qualityMetrics.ber.toFixed(2);
    document.getElementById("capacity-value").textContent =
      this.calculateCapacity();
    document.getElementById("robustness-value").textContent =
  ...
  calculateRobustness() {
    ...
    const qualityFactor = this.qualityMetrics
      ? (this.qualityMetrics.psnr / 40 + this.qualityMetrics.ssim) / 2
      : 1;

    return Math.min(
      95,
      (baseRobustness[this.currentAlgorithm] || 0) * qualityFactor,
    );
  }
  ...
  async testCompression() {
    ...
    for (const quality of compressionLevels) {
      const compressed = await this.simulateCompression(
        this.watermarkedImage,
        quality,
      );
      const extracted = await this.core.detection.blind_decoding(
        compressed,
        this.currentAlgorithm,
      );
      const ber = this.calculateBER(
        this.extractedMessage,
  ...
  async testStatisticalDetection() {
    ...
    const result = this.core.detection.statistical_detection(
      this.watermarkedImage,
    );
  ...
  async testBlindDecoding() {
    ...
    const result = await this.core.detection.blind_decoding(
      this.watermarkedImage,
      this.currentAlgorithm,
    );
  ...
  async autoDetectAlgorithm() {
    ...
    document.getElementById("extraction-algorithm").value =
      result.algorithm || "dct";
    this.showMessage(
      `Detected algorithm: ${result.algorithm || "DCT"}`,
      "success",
    );
  }
                    ...
                    ${results
                      .map(
                        ...
                        <tr>
                            ...
                            <td style="padding: 8px; border: 1px solid var(--border); text-align: center; color: ${
