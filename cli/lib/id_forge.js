...
if (require.main === module) {
  ...
  try {
    ...
    let ids;
    switch (type) {
      case 'uuidv4': ids = single ? [uuidv4()] : uuidv4Bulk(count); break;
      case 'uuidv7': ids = single ? [uuidv7()] : uuidv7Bulk(count); break;
      case 'ulid': ids = single ? [ulid()] : ulidBulk(count); break;
      case 'nanoid': ids = single ? [nanoid()] : nanoidBulk(count); break;
      case 'swhid':
        if (!arg || arg.startsWith('--')) { console.error('Usage: node id_forge.js swhid <file> [--json|--csv]'); process.exit(1); }
        ids = [swhid(arg)]; break;
      case 'all':
