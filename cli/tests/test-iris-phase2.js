...
async function main() {
  ...
  const edge = {
    imageData: gray(W, H, 128),
    width: W,
    height: H,
    roi: null,
    pupil: { x: 20, y: 20, radius: 10 },
    iris: { x: 20, y: 20, radius: 30 },
    mask,
  };
  ...
  const center = {
    ...
    height: H,
    roi: null,
    pupil: { x: 50, y: 50, radius: 12 },
    iris: { x: 50, y: 50, radius: 30 },
    mask,
  };
  const gCenter = IrisQualityFull.evaluateAcquisitionGates(center);
  assert.ok(
    !gCenter.failures.some((f) => f.startsWith("marginAdequacy")),
    "centered iris must not fail marginAdequacy: " +
      gCenter.failures.join("; "),
  );
  assert.ok(
    "marginAdequacy" in gCenter.metrics,
    "metrics must include marginAdequacy",
  );
  assert.strictEqual(
    gCenter.metrics.marginAdequacy,
    100,
    "centered margin score = 100",
  );

  // ── 2. Illumination detection ──
  const mono = IrisQualityFull.detectIllumination(
    rgba(10, 10, [120, 120, 120]),
    10,
    10,
  );
  assert.strictEqual(
    mono.modality,
    "monochrome",
    "equal channels => monochrome",
  );
  assert.strictEqual(mono.colorCapture, false);
  const color = IrisQualityFull.detectIllumination(
    rgba(10, 10, [200, 40, 10]),
    10,
    10,
  );
  ...
  const rL = await store._rehydrate({
    id: "t1",
    label: "L",
    eyeSide: "left",
    leftCode: [1, 2, 3],
  ...
  const rR = await store._rehydrate({
    id: "t2",
    label: "R",
    eyeSide: "right",
    leftCode: [1, 2, 3],
  ...
  const rU = await store._rehydrate({
    id: "t3",
    label: "U",
    leftCode: [1, 2, 3],
    leftMask: [1, 1, 1],
  ...
  assert.strictEqual(
    rU.eyeSide,
    "unknown",
    "missing eyeSide defaults to unknown",
  );
