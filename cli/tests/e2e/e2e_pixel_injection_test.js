...
describe('E2E — Pixel Injection', () => {
  ...
  it('should embed a secret into a PNG with Enhanced LSB and show result', async () => {
    ...
    assert.ok(outputHtml.includes('img') && outputHtml.includes('Pixel Injected'),
  ...
  it('should round-trip Enhanced LSB: embed then extract recovers secret', async () => {
    ...
    assert.ok(outputHtml.includes('PI ROUNDTRIP'),
  ...
  it('should round-trip DCT: embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      ...
      if (cat) { cat.value = 'frequency'; cat.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'dct'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    ...
    assert.ok(outputHtml.includes('OK'),
  ...
  it('should embed with DWT (frequency domain) and show watermarked image', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'dwt'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip Multi-Channel LSB: embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const cat = document.getElementById('pi-category');
      if (cat) { cat.value = 'spatial'; cat.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'multi_channel_lsb'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    ...
    assert.ok(outputHtml.includes('MC ROUNDTRIP'),
  ...
  it('should round-trip DFT: embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'dft'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip Hybrid DCT-DWT: embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'hybrid_dct_dwt'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip Random LSB: embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'random_lsb'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    ...
    assert.ok(outputHtml.includes('RANDOM LSB'),
  ...
  it('should round-trip VINE (deep learning): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const cat = document.getElementById('pi-category');
      if (cat) { cat.value = 'deep_learning'; cat.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'vine'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    ...
    assert.ok(outputHtml.includes('OK'),
      'VINE extract should recover secret. Got: ' + outputHtml.substring(0, 300));
    await ctx.close();
  });
  ...
  it('should round-trip Pixel Seal (deep learning): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'pixel_seal'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip NullGuard (deep learning): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'nullguard'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip Shallow Diffuse (deep learning): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'shallow_diffuse'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    ...
    assert.ok(imgDataUrl && imgDataUrl.startsWith('data:'), 'Shallow Diffuse watermarked image URL should be available');
    const base64Data = imgDataUrl.split(',')[1];
    const imgBuf = Buffer.from(base64Data, 'base64');
  ...
  it('should round-trip Imagewmark Pro (professional): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const cat = document.getElementById('pi-category');
      if (cat) { cat.value = 'professional'; cat.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'imagewmark'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip Meta Seal (professional): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'meta_seal'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip STARDUSTmark (professional): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'stardustmark'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip InvisMark (professional): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'invisimark'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip ElevenLikes (professional): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'elevenlikes'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
  ...
  it('should round-trip Diffusion-based (professional): embed then extract recovers secret', async () => {
    ...
    await page.evaluate(() => {
      const sel = document.getElementById('pi-algorithm');
      if (sel) { sel.value = 'diffusion_based'; sel.dispatchEvent(new Event('change', { bubbles: true })); }
    });
    await page.waitForTimeout(300);
    ...
    assert.ok(imgDataUrl && imgDataUrl.startsWith('data:'), 'Diffusion-based watermarked image URL should be available');
    const base64Data = imgDataUrl.split(',')[1];
    const imgBuf = Buffer.from(base64Data, 'base64');
