...
describe('E2E — C2PA Provenance', () => {
  ...
  it('should read a PNG and show C2PA result (or empty state)', async () => {
    ...
    assert.ok(outputHtml.includes('C2PA') || outputHtml.includes('No') || outputHtml.includes('not'),
  ...
  it('should sign an image (write) then read back C2PA provenance data', async () => {
    ...
    assert.ok(readOutput.includes('E2E Test Image') || readOutput.includes('Active Manifest'),
      'Should show C2PA provenance. Output: ' + readOutput.substring(0, 200));
    assert.ok(readOutput.includes('Actions') || readOutput.includes('Signature'),
  ...
  it('should verify a C2PA-signed image', async () => {
    ...
    assert.ok(verifyOutput.includes('C2PA') || verifyOutput.includes('Manifest') ||
      verifyOutput.includes('valid') || verifyOutput.includes('signature'),
