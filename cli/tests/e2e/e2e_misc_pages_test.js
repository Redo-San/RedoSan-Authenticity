...
describe('E2E — Search, Assistant, About, Privacy, Contact, Social', () => {
  ...
  it('should navigate to about page and show content', async () => {
    ...
    assert.ok(content.includes('RedoSan') || content.includes('Authenticity') || content.includes('open source'),
  ...
  it('should navigate to privacy page and show content', async () => {
    ...
    assert.ok(/سياسة|خصوصية|data|privacy|datenschutz/i.test(content),
  ...
  it('should navigate to contact page and show content', async () => {
    ...
    assert.ok(content.includes('GitHub') || content.includes('issues'),
  ...
  it('should navigate to social page and show links', async () => {
    ...
    const links = await page.evaluate(() => {
      ...
      if (!section) return [];
      return Array.from(section.querySelectorAll('a')).map(a => a.href).filter(Boolean);
    });
  ...
  it('should search for "watermark" and show results', async () => {
    ...
    assert.ok(resultsHtml.includes('watermark') || resultsHtml.includes('Results'),
