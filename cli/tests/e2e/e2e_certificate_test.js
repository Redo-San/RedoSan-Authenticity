...
async function clickGenerate(page) {
  await page.evaluate(function () {
    var b = document.getElementById("cert-gen-btn");
    if (b) b.click();
  });
...
describe("E2E — Certificate / Digital Passport", { timeout: 600000 }, () => {
  ...
  it("should generate certificate and show download links", async () => {
    ...
    if (dlVisible) {
      ...
      assert.ok(
        btnCount >= 3,
        "Should show at least 3 download buttons (got " + btnCount + ")",
      );
    }
  ...
  it("should generate a certificate (or show status from UI flow)", async () => {
    ...
    assert.ok(
      hasResult,
      'Certificate should produce status. Status: "' +
        statusText +
        '"' +
        ", DL: " +
        dlVisible,
  ...
  it("should show missing required fields error when name is empty", async () => {
    ...
    assert.ok(
      statusText.includes("Please fill in all required fields"),
      'Status: "' + statusText + '"',
    );
    await closeCovPage(page, ctx);
  });
  ...
  it("should show website warning for invalid URL", async () => {
    ...
    assert.equal(fatal.length, 0, "No fatal errors: " + fatal.join(", "));
    assert.ok(
      visible || statusText.includes("Please fill in"),
      'Website warning or missing-fields error expected. status: "' +
        statusText +
        '"',
    );
    await closeCovPage(page, ctx);
  ...
  it("should toggle music fields visibility via checkbox", async () => {
    ...
    assert.equal(
      initDisplay,
      "none",
      "Music fields should initially be hidden",
    );
    ...
    assert.equal(
      afterCheck,
      "",
      "Music fields should be visible after checking",
    );
    ...
    assert.equal(
      afterUncheck,
      "none",
      "Music fields should be hidden after uncheck",
    );
    await closeCovPage(page, ctx);
  });
  ...
  it("should initialize phone code select with country options", async () => {
    ...
    assert.ok(
      optionCount >= 40,
      "Phone code select should have 40+ options, got " + optionCount,
    );
  ...
  it("should generate certificate with all result files uploaded", async () => {
    ...
    await page.setInputFiles("#cert-result-wm", [
      {
        name: "wm.txt",
        mimeType: "text/plain",
        buffer: Buffer.from("WM OK\nAlgo: LSB"),
      },
    ]);
    await page.waitForTimeout(200);

    // Upload pixel injection result
    await page.setInputFiles("#cert-result-pi", [
      { name: "pi.txt", mimeType: "text/plain", buffer: Buffer.from("PI OK") },
    ]);
    await page.waitForTimeout(200);
    ...
    var fpJson = JSON.stringify({
      ...
      perceptual_hashes: { ahash: "f0f0f0f0" },
    });
    await page.setInputFiles("#cert-result-fp", [
      {
        name: "fp.json",
    ...
    // Upload DID result
    var didJson = JSON.stringify({
      did: "did:key:z6Mk...test",
      signature: "sig123",
    });
    await page.setInputFiles("#cert-result-did", [
      {
        name: "did.json",
        mimeType: "application/json",
        buffer: Buffer.from(didJson),
      },
    ]);
    await page.waitForTimeout(200);

    // Upload timestamp file
    await page.setInputFiles("#cert-result-ts", [
      {
        name: "proof.ots",
        mimeType: "application/octet-stream",
        buffer: Buffer.from([0x00, 0x4f, 0x70]),
      },
    ]);
    await page.waitForTimeout(200);

    // Upload document watermark result
    await page.setInputFiles("#cert-result-docw", [
      {
        name: "docw.txt",
        mimeType: "text/plain",
        buffer: Buffer.from("DocW OK"),
      },
    ]);
    await page.waitForTimeout(200);
    ...
    if (dlVisible) {
      var btnCount = await page.evaluate(function () {
        var s = document.getElementById("cert-download-section");
        return s ? s.querySelectorAll("button").length : 0;
      });
      assert.ok(
        btnCount >= 3,
        "Should show at least 3 download buttons (got " + btnCount + ")",
  ...
  it("should show alert when attempting to download before generating", async () => {
    ...
    assert.ok(
      alertMsg.includes("Please generate the certificate first"),
      'Alert: "' + alertMsg + '"',
    );
    await closeCovPage(page, ctx);
  });
  ...
  it("should reset form fields and hide download section", async () => {
    ...
    // Reset
    await page.evaluate(function () {
      if (typeof resetProfessionalCert === "function") resetProfessionalCert();
    });
    await page.waitForTimeout(500);
    ...
    var dlDisplay = await page.evaluate(function () {
      var el = document.getElementById("cert-download-section");
    ...
    var optCount = await page.evaluate(function () {
      var sel = document.getElementById("cert-phonecode");
      return sel ? sel.options.length : 0;
    });
    assert.ok(
      optCount >= 40,
      "Phone code should have options after reset, got " + optCount,
    );

    // Music checkbox unchecked and fields hidden
    var musicChecked = await page.evaluate(function () {
      var el = document.getElementById("cert-show-music");
      return el ? el.checked : false;
    });
    assert.equal(musicChecked, false, "Music checkbox should be unchecked");
    var musicDisplay = await page.evaluate(function () {
      var el = document.getElementById("cert-music-fields");
      return el ? el.style.display : "";
    });
  ...
  it("should download certificate OTS proof via downloadCertOtsProof", async () => {
    ...
    await page.evaluate(function (proof) {
      ...
      if (!btn) {
        ...
        btn.onclick = downloadCertOtsProof;
  ...
  it("should have downloadCert function defined globally", async () => {
    ...
    var exists = await page.evaluate(function () {
      return typeof downloadCert === "function";
    });
  ...
  it("should show and hide loading overlay via showCertOverlay and hideCertOverlay", async () => {
    ...
    var showResult = await page.evaluate(function () {
      if (typeof showCertOverlay !== "function")
        return "showCertOverlay not defined";
      showCertOverlay();
      var hasText =
        document.body.textContent.indexOf("Generating certificate") !== -1;
      var hasSpinStyle = !!document.getElementById("cert-spin-style");
      var hasStyleContent = hasSpinStyle
        ? document
            .getElementById("cert-spin-style")
            .textContent.indexOf("certSpin") !== -1
        : false;
      return {
        hasText: hasText,
        hasSpinStyle: hasSpinStyle,
    ...
    assert.ok(
      showResult.hasText,
      "Overlay text should exist after showCertOverlay",
    );
    assert.ok(
      showResult.hasSpinStyle,
      "cert-spin-style element should exist after showCertOverlay",
    );
    assert.ok(
      showResult.hasStyleContent,
      "cert-spin-style should contain certSpin animation",
    );
    ...
    var afterHide = await page.evaluate(function () {
      if (typeof hideCertOverlay !== "function")
        return "hideCertOverlay not defined";
      hideCertOverlay();
      var textGone =
        document.body.textContent.indexOf("Generating certificate") === -1;
    ...
    assert.ok(
      afterHide.textGone,
      "Overlay text should be removed after hideCertOverlay",
    );
  ...
  it("should collect certificate data from window globals via collectCertData", async () => {
    ...
    var result = await page.evaluate(async function () {
      ...
      window.simpleUserInfo = {
        ...
        isArtist: true,
        music: { spotify: "https://spotify.com/track/1" },
      };
      window.simpleResults = {
        watermark: true,
        watermarkUrl: "data:text/plain,wm",
        watermarkAlgoName: "DCT",
        watermarkResult: "OK",
        "pixel-injection": true,
        piResultHtml: "<b>PI Success</b>",
        timestamp: true,
        tsResult: "TS verified",
        fingerprint: true,
        fpResult: {
          hashes: {
            "SHA-256":
      ...
      window.simpleFile = new File([content], "photo.png", {
        type: "image/png",
      });
      ...
      window.submitCertTransparency = async function () {
        return {
          submitted: true,
          pending: true,
          otsProof: "bW9jay1vdHM=",
          hash: "ab",
          timestamp: new Date().toISOString(),
        };
      };

      if (typeof collectCertData !== "function")
        return { error: "collectCertData not defined" };
    ...
    assert.equal(
      result.generatorPrefix,
      "RedoSa",
      "Generator should start with RedoSan",
    );
    ...
    assert.ok(
      result.didIdentity.length > 0,
      "DID identity should be non-empty",
    );
    assert.ok(result.ctSubmitted, "CT should be submitted");
    assert.equal(
      result.fileWidth,
      0,
      "File width should be 0 for non-image data",
    );
  ...
  it("should stamp a certificate blob via stampCertFile and show OTS button", async () => {
    ...
    var stampResult = await page.evaluate(async function () {
      ...
      if (typeof stampCertFile !== "function")
        return { error: "stampCertFile not defined" };

      var blob = new Blob(["test-certificate-pdf-content"], {
    ...
    assert.equal(
      stampResult.btnDisplay,
      "inline-block",
      "OTS button should be visible after stampCertFile",
    );
  ...
  it("should generate PDF via downloadCert in simple mode with mocked dependencies", async () => {
    ...
    await page.evaluate(function () {
      ...
      window.downloadCertPDF = async function () {
        return new Blob(["mock-pdf-from-downloadCert"], {
          type: "application/pdf",
        });
      };
    ...
    if (dlError) {
      assert.fail(
        "downloadCert threw: " +
          dlError +
          (alertCaught ? " (alert: " + alertCaught + ")" : ""),
      );
    }
    assert.ok(
      overlayGone,
      "Overlay should be hidden after downloadCert completes (alert: " +
        alertCaught +
        ")",
    );
    ...
    assert.equal(
      otsDisplay,
      "inline-block",
      "cert-ots-dl-btn should be visible",
    );
...
describe(
  "E2E — Certificate / Digital Passport (MPA)",
  { timeout: 600000 },
  () => {
    ...
    it("should load MPA page without errors", async () => {
      ...
      await page.waitForTimeout(2000);
      var fatal = errors.filter(function (e) {
        return !e.includes("404") && !e.includes("Failed to load");
      });
      assert.equal(fatal.length, 0, "No fatal errors: " + fatal.join(", "));
      await closeCovPage(page, ctx);
    });
    ...
    it("should have form fields and generate button (MPA)", async () => {
      ...
      assert.ok(els.email);
      assert.ok(els.phone);
      assert.ok(els.website);
      assert.ok(els.btn);
      assert.ok(els.phoneCode);
      assert.ok(els.showMusic);
      await closeCovPage(page, ctx);
    });
    ...
    it("should show website warning for invalid URL (MPA)", async () => {
      ...
      assert.ok(
        visible || statusText.includes("Please fill in"),
        'Warning or error expected. status: "' + statusText + '"',
      );
      await closeCovPage(page, ctx);
    });
    ...
    it("should toggle music fields via checkbox (MPA)", async () => {
      ...
      await page.evaluate(function () {
        ...
        if (typeof toggleCertMusicFields === "function")
    ...
    it("should generate certificate with result files and show download section (MPA)", async () => {
      ...
      await page.waitForTimeout(200);
      await page.setInputFiles("#cert-result-wm", [
        { name: "wm.txt", mimeType: "text/plain", buffer: Buffer.from("OK") },
      ]);
      await page.waitForTimeout(200);
      await page.setInputFiles("#cert-result-fp", [
        {
          name: "fp.json",
          mimeType: "application/json",
          buffer: Buffer.from(JSON.stringify({ hashes: { "SHA-256": "abc" } })),
        },
      ]);
      await page.waitForTimeout(200);
      ...
      var dlVisible = await page.evaluate(function () {
        var el = document.getElementById("cert-download-section");
        return el
          ? el.style.display !== "none" && el.style.display !== ""
          : false;
    ...
    it("should reset form and hide download section (MPA)", async () => {
      ...
      await page.evaluate(function () {
        if (typeof resetProfessionalCert === "function")
    ...
    it("should have phone code select populated with country options (MPA)", async () => {
      ...
      assert.ok(
        count >= 40,
        "Phone code should have 40+ options, got " + count,
      );
      await closeCovPage(page, ctx);
    });
    ...
    it("should download professional certificate PDF successfully with stubs and trigger stampCertFile", async () => {
      ...
      var downloadCheck = await page.evaluate(function () {
        ...
        window.downloadCertPDF = async function () {
          return new Blob(["mock-professional-pdf"], {
            type: "application/pdf",
          });
        };
      ...
      if (dlVisible) {
        ...
        if (alertMsg) {
          ...
          assert.ok(
            alertMsg.indexOf("Error") === -1,
            "No error alert: " + alertMsg,
          );
        }
        ...
        assert.equal(
          otsDisplay,
          "inline-block",
          "cert-ots-dl-btn should be visible after stampCertFile",
        );
...
describe("E2E — Certificate expanded coverage", () => {
  ...
  it("should create fallback script tags in ensureLib when global is temporarily missing", async () => {
    ...
    var tagsCreated = await page.evaluate(async function () {
      ...
      var before = document.head.querySelectorAll(
        'script[src*="jspdf"], script[src*="jsPDF"]',
      ).length;
      try {
        await Promise.race([
          ensureLib("jspdf"),
        ...
        /* CDN unreachable — expected */
      }
      var after = document.head.querySelectorAll(
        'script[src*="jspdf"], script[src*="jsPDF"]',
      ).length;
      window.jspdf = saved;
      return after - before;
    ...
    assert.ok(
      tagsCreated >= -2,
      "ensureLib fallback path executed (tagsAdded=" + tagsCreated + ")",
    );
    if (tagsCreated > 0) {
      assert.ok(
        tagsCreated >= 1,
        "ensureLib created " + tagsCreated + " fallback script tag(s)",
      );
    }
  ...
  it("should call downloadCert with DOCX format and button parameter in simple mode", async () => {
    ...
    await page.evaluate(async function (bufArr) {
      ...
      window.generatePendingOts = function () {
        return null;
      };
      if (typeof jspdf === "undefined") {
        window.jspdf = {
  ...
  it("should generate a REAL PDF via downloadCert with vendored jspdf/QRious and real OTS proof", async () => {
    ...
    assert.ok(
      libs.realGenerators,
      "real downloadCertPDF / generatePendingOts should exist",
    );
    ...
    await page.evaluate(function () {
      ...
      window._faceData = null;
      var encoder = new TextEncoder();
      window.simpleFile = new File(["real-image-bytes"], "real.png", {
        type: "image/png",
      });
    ...
    assert.equal(
      dlError,
      "",
      "downloadCert(pdf) should not throw (alert: " + alertMsg + ")",
    );
    ...
    assert.ok(
      ctResult.otsProof,
      "REAL generatePendingOts should produce an OTS proof (base64)",
    );
    assert.ok(
      ctResult.otsProof.length > 20,
      "OTS proof should be a real serialized proof, got " +
        ctResult.otsProof.length +
        " chars",
    );
    assert.equal(ctResult.format, "pdf", "Format should be pdf");
    ...
    assert.equal(
      pdfInfo.type,
      "application/pdf",
      "Real downloadCertPDF should return a PDF blob",
    );
    assert.ok(
      pdfInfo.size > 5000,
      "Real PDF blob should be substantial, got " + pdfInfo.size + " bytes",
    );
  ...
  it("should generate REAL DOCX and EPUB blobs via the actual generator functions", async () => {
    ...
    assert.ok(
      fs.existsSync(docxIife),
      "node_modules/docx/dist/index.iife.js must exist for the real DOCX test",
    );
    ...
    await page.evaluate(function () {
      window.simpleUserInfo = {
        name: "Real Docx",
        email: "docxreal@test.com",
        phone: "+1-555-7776665",
        website: "https://docxreal.com",
        social: {},
        isArtist: true,
      ...
      window._faceData = null;
      var encoder = new TextEncoder();
      window.simpleFile = new File(["docx-image"], "docx.png", {
        type: "image/png",
      });
    ...
    assert.ok(
      blobInfo.docx.type.indexOf("wordprocessingml") !== -1,
      "DOCX blob should be an OOXML document, got " + blobInfo.docx.type,
    );
    assert.ok(
      blobInfo.docx.size > 1000,
      "Real DOCX blob should be substantial, got " +
        blobInfo.docx.size +
        " bytes",
    );
    assert.ok(blobInfo.epub, "downloadCertEPUB should have returned a blob");
    assert.ok(
      blobInfo.epub.type.indexOf("zip") !== -1,
      "EPUB blob should be a zip archive (JSZip output), got " +
        blobInfo.epub.type,
    );
    assert.ok(
      blobInfo.epub.size > 1000,
      "Real EPUB blob should be substantial, got " +
        blobInfo.epub.size +
        " bytes",
    );
    ...
    var dataUrlResult = await page.evaluate(function () {
      ...
      return typeof url === "string" && url.indexOf("blob:") === 0
        ? "ok"
        : "bad:" + String(url).slice(0, 30);
    });
    assert.equal(
      dataUrlResult,
      "ok",
      "makeCertDataURL should return a blob: URL",
    );
