...
describe("MPA — Fingerprint", function () {
  ...
  it("should have key form elements", async function () {
    ...
    try {
      var hasFile = await page.evaluate(function () { return !!document.getElementById("fp-file"); });
      var hasBtn = await page.evaluate(function () { return !!document.getElementById("fp-btn"); });
  ...
  it("should run fingerprint on uploaded image and show result", async function () {
    ...
    try {
      ...
      assert.ok(outputHtml.length > 0, "Output should contain fingerprint result");
    } finally {
      await closePage(ctx, page);
    }
  });
