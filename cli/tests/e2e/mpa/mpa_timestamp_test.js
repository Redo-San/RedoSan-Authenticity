...
describe("MPA — Timestamp", function () {
  ...
  it("should have key form elements", async function () {
    ...
    try {
      var hasFile = await page.evaluate(function () { return !!document.getElementById("ts-create-file"); });
      var hasBtn = await page.evaluate(function () { return !!document.getElementById("ts-create-btn"); });
  ...
  it("should create timestamp from uploaded file and show result", async function () {
    ...
    try {
      ...
      assert.ok(outputHtml.length > 0, "Output should contain timestamp result");
    } finally {
      await closePage(ctx, page);
    }
  });
