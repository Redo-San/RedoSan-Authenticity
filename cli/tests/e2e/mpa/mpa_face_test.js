...
describe("MPA — Face Biometric", function () {
  ...
  it("should have key form elements", async function () {
    ...
    try {
      var hasInput = await page.evaluate(function () {
        return !!document.getElementById("face-image");
      });
      var hasLabel = await page.evaluate(function () {
        return !!document.getElementById("face-label");
      });
      var hasRun = await page.evaluate(function () {
        return !!document.getElementById("face-run");
      });
      var hasEmbedder = await page.evaluate(function () {
        return !!document.getElementById("face-embedder");
      });
  ...
  it("should list both embedder options (human + arcface)", async function () {
    ...
    try {
      ...
      assert.ok(
        opts.indexOf("human") !== -1,
        "human embedder option should exist",
      );
      assert.ok(
        opts.indexOf("arcface") !== -1,
        "arcface embedder option should exist",
      );
    } finally {
      await closePage(ctx, page);
    }
  ...
  it("should expose the ArcFace ONNX embedder module", async function () {
    ...
    try {
      var has = await page.evaluate(function () {
        return (
          typeof window.FaceONNXEmbedder === "object" &&
  ...
  it("should inject page CSS and run init when navigated via __mpaNavigate", async function () {
    ...
    try {
      ...
      var state = await page.evaluate(function () {
        ...
        return {
          cssLoaded: links.some(function (l) {
            return (
              (l.href || "").indexOf("face-biometric/css/style.css") !== -1
            );
          }),
          panelVisible:
            getComputedStyle(document.getElementById("face-consent-panel"))
              .display !== "none",
      ...
      assert.equal(
        state.cssLoaded,
        true,
        "page css should be injected after AJAX navigation",
      );
      assert.equal(
        state.panelVisible,
        true,
        "consent panel should be visible without consent",
      );
      assert.equal(
        state.runDisabled,
        true,
        "run button should be disabled until consent",
      );
      ...
      assert.equal(
        acceptDisabled,
        false,
        "accept button should enable after ticking the checkbox",
      );
      await page.evaluate(function () {
      ...
      assert.equal(
        after.saved,
        true,
        "consent should be saved to sessionStorage",
      );
      assert.equal(
        after.notPersisted,
        true,
        "consent must not persist in localStorage",
      );
      assert.equal(
        after.panelHidden,
        true,
        "consent panel should hide after accept",
      );
      assert.equal(
        after.inputEnabled,
        true,
        "file input should enable after consent",
      );
    } finally {
