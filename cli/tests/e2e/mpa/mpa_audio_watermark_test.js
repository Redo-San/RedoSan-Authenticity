...
describe("MPA — Audio Watermark", function () {
  ...
  it("should have key form elements", async function () {
    ...
    try {
      var hasAudio = await page.evaluate(function () {
        return !!document.getElementById("awm-audio");
      });
      var hasType = await page.evaluate(function () {
        return !!document.getElementById("awm-type");
      });
      var hasBtn = await page.evaluate(function () {
        return !!document.getElementById("awm-btn");
      });
  ...
  it("should embed a watermark using LSB algorithm", async function () {
    ...
    try {
      ...
      await page.setInputFiles("#awm-file", [
        {
          name: "secret.txt",
          mimeType: "text/plain",
          buffer: Buffer.from("E2E TEST MESSAGE"),
        },
      ]);
      await page.waitForTimeout(300);
      ...
      await page.evaluate(function () {
        document.getElementById("awm-btn").click();
      });
      ...
      assert.ok(
        resultDiv === "block" || resultDiv === "" || progDone === "none",
        "Watermark embed should complete (result display: " +
          resultDiv +
          ", progress: " +
          progDone +
          ")",
  ...
  it("should switch to extract tab and have extract and dual elements", async function () {
    ...
    try {
      await page.evaluate(function () {
        switchAwmTab("extract");
      });
      await page.waitForTimeout(500);
      var hasExtractBtn = await page.evaluate(function () {
        return !!document.getElementById("awm-btn-ex");
      });
      var hasAudioEx = await page.evaluate(function () {
        return !!document.getElementById("awm-audio-ex");
      });
      assert.ok(
        hasExtractBtn,
        "Extract button should exist after switching tab",
      );
      assert.ok(hasAudioEx, "Extract audio input should exist");

      // Also check dual extract elements exist
      var hasFpAlgo = await page.evaluate(function () {
        return !!document.getElementById("awm-dual-fp-algo");
      });
      var hasDidAlgo = await page.evaluate(function () {
        return !!document.getElementById("awm-dual-did-algo");
      });
  ...
  it("should round-trip LSB: embed then extract recovers message", async function () {
    ...
    try {
      ...
      await page.setInputFiles("#awm-file", [
        {
          name: "secret.txt",
          mimeType: "text/plain",
          buffer: Buffer.from("ROUNDTRIP TEST LSB"),
        },
      ]);
      await page.waitForTimeout(300);
      ...
      await page.waitForTimeout(500);
      await page.evaluate(function () {
        document.getElementById("awm-btn-ex").click();
      });
      ...
      assert.ok(
        extractHtml.includes("ROUNDTRIP TEST LSB"),
        "Extract should recover original message. Got: " +
