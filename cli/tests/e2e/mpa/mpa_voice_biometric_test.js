...
var MODELS_DIR = path.resolve(__dirname, "../../../Voice_Biometric/models");
var browser;
var tmpDir;
var opened;
var savedState;
...
async function saveState() {
  if (opened && opened.ctx) {
    ...
    // Playwright storageState({ indexedDB: true }) silently fails on some
    // Chromium versions (never captures IndexedDB). Always merge IndexedDB
    // data from the manual localStorage backup into the Playwright state.
    var state;
    try {
      state = await opened.ctx.storageState({ indexedDB: true });
      console.log(
        "[STATE] storageState returned " +
    ...
    // Merge manual IndexedDB backup into the state
    var manual = await manualBackupIndexedDB();
    if (manual && manual.origins) {
      // Find or create the localhost origin in the state
      var manualOrigin = manual.origins.find(function (o) {
        return o.origin === "http://localhost:9455";
      });
      if (
        manualOrigin &&
        manualOrigin.indexedDB &&
        manualOrigin.indexedDB.VoiceRegistryDB
      ) {
        if (!state.origins) state.origins = [];
        var stateOrigin = state.origins.find(function (o) {
          return o.origin === "http://localhost:9455";
        });
        if (!stateOrigin) {
          stateOrigin = {
          ...
          state.origins.push(stateOrigin);
        }
        stateOrigin.indexedDB = manualOrigin.indexedDB;
        console.log(
          "[STATE] Merged IndexedDB into storageState (" +
            (stateOrigin.indexedDB.VoiceRegistryDB.templates || []).length +
            " templates)",
        );
      }
    }
    ...
    if (savedState && savedState.origins) {
      savedState.origins.forEach(function (o) {
        if (o.indexedDB && o.indexedDB.VoiceRegistryDB) hasIdb = true;
      });
    }
    console.log(
      "[STATE] savedState set, origins=" +
        (savedState.origins || []).length +
        ", hasIndexedDB=" +
        hasIdb,
    );
  }
...
var ROUTE_MAP = {
  "huggingface.co/vedk00/ecapa-voxceleb-speaker-embedding-onnx/resolve/main/model/ecapa-speaker-v1.onnx":
    "ecapa-speaker-v1.onnx",
  "huggingface.co/D4ve-R/wavlm-base-plus-sv/resolve/main/onnx/model_quantized.onnx":
    "model_quantized.onnx",
  "huggingface.co/runanywhere/silero-vad-v5/resolve/main/silero_vad.onnx":
    "silero_vad.onnx",
  "huggingface.co/SpeechAntiSpoofingBenchmarks/AASIST-L/resolve/main/aasist-l.onnx":
    "aasist-l.onnx",
  "huggingface.co/SpeechAntiSpoofingBenchmarks/AASIST/resolve/main/aasist.onnx":
    "aasist.onnx",
  "cdn.jsdelivr.net/npm/onnxruntime-web@1.20.1/dist/ort.min.js": "ort.min.js",
  "cdn.jsdelivr.net/npm/onnxruntime-web@1.20.1/dist/ort-wasm-simd-threaded.wasm":
    "ort-wasm-simd-threaded.wasm",
  "cdn.jsdelivr.net/npm/onnxruntime-web@1.20.1/dist/ort-wasm-simd-threaded.jsep.wasm":
    "ort-wasm-simd-threaded.jsep.wasm",
};

function setupRouteInterception(page) {
  return page.route("**/*", function (route) {
    var url = route.request().url();
    for (var pattern in ROUTE_MAP) {
      if (url.indexOf(pattern) !== -1) {
        var localFile = path.join(MODELS_DIR, ROUTE_MAP[pattern]);
        if (fs.existsSync(localFile)) {
          ...
          else if (ext === ".onnx") ct = "application/octet-stream";
...
async function openConsented() {
  ...
  if (savedState && savedState.origins) {
    ...
    var idbData =
      origin && origin.indexedDB && origin.indexedDB.VoiceRegistryDB;
    var tmplCount = idbData && idbData.templates ? idbData.templates.length : 0;
    console.log(
      "[RESTORE] savedState has " +
        savedState.origins.length +
        " origins, " +
        tmplCount +
...
async function runFile(page, file, label) {
  ...
  await page
    .evaluate(function () {
      ...
      var overlay = document.getElementById("voice-progress-overlay");
      if (overlay) {
  ...
  await page
    .evaluate(function () {
      ...
      var bot = document.getElementById("botBlockOverlay");
      if (bot) {
  ...
  it(
    ...
    async function () {
      ...
      process.stderr.write(
        "[PROGRESS] h1a ECAPA done in " + h1aEcapa.wallMs + "ms\n",
      );
      ...
      h1b = await runFile(page, "golden_h1b.wav", "e2e-h1b");
      process.stderr.write(
        "[PROGRESS] h1b ECAPA match done in " + h1b.wallMs + "ms\n",
      );
      ...
      h2 = await runFile(page, "golden_h2.wav", "e2e-h2");
      process.stderr.write(
        "[PROGRESS] h2 ECAPA done in " +
          h2.wallMs +
          "ms, similarity=" +
          h2.report.speaker.similarity +
          ", decision=" +
          h2.report.speaker.decision +
          "\n",
      );
      assert.equal(h2.report.quality.gate, "PASS");
      if (h2.report.speaker.similarity !== null) {
        process.stderr.write(
          "[MEASURE] cross-speaker cos=" +
            h2.report.speaker.similarity +
            " (threshold=0.7)\n",
        );
      }

      song = await runFile(page, "golden_song.wav", "e2e-song");
      process.stderr.write("[PROGRESS] song done in " + song.wallMs + "ms\n");
      ...
      silence = await runFile(page, "silence.wav", "e2e-silence");
      process.stderr.write(
        "[PROGRESS] silence done in " + silence.wallMs + "ms\n",
      );
      ...
      process.stderr.write(
        "[PROGRESS] wavlm h1a done in " + wavHeavy.wallMs + "ms\n",
      );
      rw = wavHeavy.report;
      ...
      var out = {
        ...
        ecapa: {
          ...
          h1b: {
            similarity: h1b.report.speaker
              ? h1b.report.speaker.similarity
              : null,
            decision: h1b.report.speaker ? h1b.report.speaker.decision : null,
            embedMagnitude: h1b.report.speaker
              ? h1b.report.speaker.embedMagnitude
              : null,
            timings: h1b.report.timings,
            wallMs: h1b.wallMs,
          },
          h2: {
            similarity: h2.report.speaker ? h2.report.speaker.similarity : null,
            decision: h2.report.speaker ? h2.report.speaker.decision : null,
            timings: h2.report.timings,
            wallMs: h2.wallMs,
          },
          songGate: song.report.quality.gate,
          songMagnitude: song.report.speaker
            ? song.report.speaker.embedMagnitude
            : null,
        },
        wavlm: {
          h1a: {
