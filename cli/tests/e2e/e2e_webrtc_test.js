...
describe("E2E — WebRTC / VPN Detection", () => {
  it("should set REDOSAN_BOT_CHECK after page load", async () => {
    ...
    const botCheck = await page.evaluate(() => {
      const b =
        typeof REDOSAN_BOT_CHECK !== "undefined" ? REDOSAN_BOT_CHECK : null;
      return b
        ? {
            score: b.score,
            isAutomated: b.isAutomated,
            signalCount: b.signals ? b.signals.length : 0,
          }
        : null;
    });
    ...
    assert.ok(
      botCheck !== null,
      "REDOSAN_BOT_CHECK should be a non-null object",
    );
    assert.equal(typeof botCheck.score, "number", "score should be a number");
    assert.ok(
      botCheck.score >= 0 && botCheck.score <= 100,
  ...
  it("should detect headless Chromium as automated (score >= 40)", async () => {
    ...
    assert.equal(
      automated,
      true,
      "Headless Chromium should be detected as automated",
    );
    await ctx.close();
  });
  ...
  it("should include 'webdriver' signal for headless Chromium", async () => {
    ...
    assert.ok(
      hasWebdriver,
      "headless Chromium should trigger 'webdriver' signal",
    );
    await ctx.close();
  });
  ...
  it("should show bot overlay in headless mode", async () => {
    ...
    assert.ok(
      overlayVisible,
      "botBlockOverlay should have 'active' class in headless mode",
    );
    await ctx.close();
  });
  ...
  it("should have Access Denied message in the bot overlay", async () => {
    ...
    assert.ok(
      overlayText.title.includes("Access Denied") ||
        overlayText.title.includes("رفض"),
      "Overlay title should indicate access denial",
    );
    assert.ok(
      overlayText.text.includes("headless") ||
        overlayText.text.includes("automation") ||
        overlayText.text.includes("أتمتة"),
  ...
  it("should call startAsyncVPNDetection (may be skipped in headless)", async () => {
    ...
    const fnExists = await page.evaluate(
      () => typeof startAsyncVPNDetection === "function",
    );
    assert.ok(fnExists, "startAsyncVPNDetection function should exist");

    const detectWebRTCExists = await page.evaluate(
      () => typeof detectWebRTCIPs === "function",
    );
    ...
    const score = await page.evaluate(() =>
      REDOSAN_BOT_CHECK ? REDOSAN_BOT_CHECK.score : -1,
    );
  ...
  it("should not block non-automated access (backstop signal bypass)", async () => {
    ...
    const botCheck = await page.evaluate(() => {
      return REDOSAN_BOT_CHECK
        ? {
            isAutomated: REDOSAN_BOT_CHECK.isAutomated,
            signals: REDOSAN_BOT_CHECK.signals,
          }
        : null;
    });
    ...
    assert.ok(
      botCheck.signals.includes("backstop"),
      "Should contain backstop signal",
    );
    assert.equal(
      botCheck.isAutomated,
      false,
      "Backstop bypass should result in isAutomated=false",
    );
    ...
    assert.equal(
      overlayActive,
      false,
      "Bot overlay should NOT be active for backstop bypass",
    );
  ...
  it("should not produce fatal console errors during detection", async () => {
    ...
    assert.equal(
      fatal.length,
      0,
      `Fatal errors during detection: ${fatal.join(", ")}`,
    );
    await ctx.close();
  });
});
