...
describe('E2E — Converter & Removal Tools', () => {
  ...
  it('should convert PNG to JPEG and produce download', async () => {
    ...
    await page.evaluate(() => {
      ...
      for (const b of btns) {
        if (b.textContent.toLowerCase().includes('jpeg')) { b.click(); break; }
      }
    });
    ...
    assert.ok(dlText.includes('.jpg') || dlText.includes('.jpeg'), 'Filename should have jpg extension. Got: ' + dlText);
