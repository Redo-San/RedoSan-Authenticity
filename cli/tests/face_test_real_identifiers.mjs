...
const imgPath = path.join(__dirname, "fixtures", "face_identifier_test_img.jpg");
...
page.on("console", (msg) => {
  ...
  if (text.includes("Human:") || text.includes("BioHash") || text.includes("Fuzzy"))
...
await page.waitForFunction(
  () => typeof window.FaceBioHash !== "undefined" && typeof window.FaceFuzzy !== "undefined",
  { timeout: 15000 }
);
console.log("Identifiers loaded: " + (await page.evaluate(() => typeof window.FaceBioHash !== "undefined" && typeof window.FaceFuzzy !== "undefined")));
...
const camWrapperVisible = await page.evaluate(() => document.getElementById("face-capture-wrapper").style.display === "block");
const uploadWrapperHidden = await page.evaluate(() => document.getElementById("face-upload-wrapper").style.display === "none");
...
if (!camWrapperVisible) console.log("FAIL: capture wrapper must show on the camera tab");
if (!uploadWrapperHidden) console.log("FAIL: upload wrapper must hide on the camera tab");
await page.selectOption("#face-liveness-mode", "off");
await page.click("#face-cam-start");
await waitForTrue(async () => (await page.textContent("#face-status") || "").includes("Camera started"), 15000, "camera start");
...
console.log("File input disabled while camera active: " + fileDisabledWhileCamera);
console.log("Capture button enabled while camera active: " + captureEnabledWhileCamera);
if (!fileDisabledWhileCamera) console.log("FAIL: file upload must be disabled while the camera is running");
if (!captureEnabledWhileCamera) console.log("FAIL: capture button must be enabled while the camera is running");

// Capture a frame (stages it, does NOT run the pipeline)
await page.click("#face-cam-capture");
await waitForTrue(async () => await page.evaluate(() => !!window._facePendingSource), 10000, "camera frame staged");
const camStaged = await page.evaluate(() => ({ source: window._facePendingSource && window._facePendingSource.source, fileName: window._facePendingSource && window._facePendingSource.fileName }));
console.log("Camera frame staged: " + JSON.stringify(camStaged));
const reportBeforeRun = await page.evaluate(() => !!window._faceReport);
console.log("Report NOT generated on capture (expected false): " + reportBeforeRun);
if (reportBeforeRun) console.log("FAIL: pipeline must not run on capture alone");

// Label empty → run stays disabled
const runDisabledAfterCapture = await page.isDisabled("#face-run");
console.log("Run button disabled after capture (no label): " + runDisabledAfterCapture);
if (!runDisabledAfterCapture) console.log("FAIL: run button must need the label field");
...
if (!runEnabledAfterLabel) console.log("FAIL: run button must enable when label is filled");
...
await page.evaluate(async () => {
  const img = new Image();
  img.src = "http://127.0.0.1:8080/cli/tests/fixtures/face_identifier_test_img.jpg";
...
const camReportOk = await waitForTrue(async () => await page.evaluate(() => !!window._faceReport), 90000, "camera pipeline report");
const camReport = await page.evaluate(() => {
  ...
  return r ? {
    ...
    signature: r.did && r.did.signature ? r.did.signature.substring(0, 12) : null,
    bioHashHex: r.biohash && r.biohash.codeHex ? r.biohash.codeHex.substring(0, 16) : null,
...
else if (camReport.source !== "camera" || camReport.fileName !== "camera_capture") console.log("FAIL: camera report source/fileName mismatch");
else if (camReport.liveness !== null) console.log("FAIL: liveness off should give null evidence");
else if (!camReport.registeredId) console.log("FAIL: auto-register should happen with a label");
...
await page.waitForTimeout(200);
const uploadWrapperVisible = await page.evaluate(() => document.getElementById("face-upload-wrapper").style.display === "block");
...
console.log("Upload wrapper visible after switching back: " + uploadWrapperVisible);
...
if (!uploadWrapperVisible) console.log("FAIL: upload wrapper must show on the upload tab");
if (!fileEnabledAfterStop) console.log("FAIL: file upload must be re-enabled after the camera stops");
if (!captureDisabledAfterStop) console.log("FAIL: capture button must be disabled after the camera stops");
...
await waitForTrue(async () => await page.evaluate(() => window._facePendingSource && window._facePendingSource.source === "file"), 10000, "file staged");
const fileStaged = await page.evaluate(() => ({ source: window._facePendingSource && window._facePendingSource.source, fileName: window._facePendingSource && window._facePendingSource.fileName }));
console.log("File staged: " + JSON.stringify(fileStaged));
const runDisabledAfterFile = await page.isDisabled("#face-run");
console.log("Run button disabled after upload (label empty): " + runDisabledAfterFile);
if (!runDisabledAfterFile) console.log("FAIL: run button must stay disabled until the label is filled");
const camStartDisabledInFileMode = await page.isDisabled("#face-cam-start");
console.log("Camera start disabled in file mode: " + camStartDisabledInFileMode);
if (!camStartDisabledInFileMode) console.log("FAIL: camera must be disabled while a photo is picked");
...
await page.click("#face-run");
const fileReportOk = await waitForTrue(async () => {
  const r = await page.evaluate(() => window._faceReport);
  return !!(r && r.source === "file");
}, 90000, "file pipeline report");
const fileReport = await page.evaluate(() => {
  ...
  return r ? {
    ...
    vcIssuer: r.did && r.did.verifiableCredential && r.did.verifiableCredential.issuer,
...
if (!fileReportOk) console.log("FAIL: no file pipeline report");
  ...
  if (fileReport.source !== "file" || fileReport.fileName !== "face_identifier_test_img.jpg") console.log("FAIL: source/fileName mismatch");
  if (fileReport.facesDetected < 1) console.log("FAIL: no face detected in the real photo");
  if (!fileReport.did || !fileReport.did.startsWith("did:key:")) console.log("FAIL: DID missing");
  if (fileReport.algorithm !== "Ed25519") console.log("FAIL: DID algorithm");
  if (!fileReport.signature) console.log("FAIL: signature missing");
  if (!fileReport.didDocId || !fileReport.vcIssuer) console.log("FAIL: DID document / VC missing");
  if (!fileReport.biohashHex || fileReport.biohashHex.length !== 32) console.log("FAIL: BioHash hex must be 32 chars");
  if (fileReport.biohashPinAuto !== true) console.log("FAIL: PIN is auto-generated now");
  if (!fileReport.fuzzyKey) console.log("FAIL: Fuzzy key missing");
  if (!fileReport.registeredId) console.log("FAIL: auto-register should have run");
}
...
await page.waitForTimeout(500);
const listCount = await page.evaluate(() => document.querySelectorAll("#face-list .face-list-item").length);
...
await page.waitForTimeout(300);
const modalOpen = await page.evaluate(() => document.getElementById("dl-modal").classList.contains("open"));
...
if (dl) { const f = path.join(__dirname, "face_identifiers_report_dl.json"); await dl.saveAs(f); console.log("Saved: " + f); }
// downloadResult() closes the modal itself; verify it did.
await page.waitForTimeout(300);
const modalClosed = await page.evaluate(() => !document.getElementById("dl-modal").classList.contains("open"));
...
await page.click('button[onclick="listRegisteredFaces()"]');
await page.waitForTimeout(500);
const afterDelete = await page.evaluate(() => document.querySelectorAll("#face-list .face-list-item").length);
...
pageErrors.slice(0, 10).forEach((e) => { console.log("[pageerror] " + e.slice(0, 200)); });

await page.screenshot({ path: path.join(__dirname, "face_identifiers_result.png"), fullPage: true });
