...
function setupDOM() {
  ...
  globalThis.document = {
        ...
        const el = {
          ...
          appendChild(child) {
            if (child && child.textContent !== undefined)
              this._txt += child.textContent;
          },
          append(...children) {
            for (const c of children) {
              if (c && c.textContent !== undefined) this._txt += c.textContent;
            }
          },
        };
        Object.defineProperty(el, "innerHTML", {
          get() {
            return this._txt || this._inner;
          },
          set(v) {
            this._inner = v;
            this._txt = "";
    ...
    createTextNode(txt) {
      return { nodeType: 3, textContent: txt };
    },
  };
...
function makeTestWav(numSamples, sr) {
  ...
  for (let i = 0; i < numSamples; i++) {
    v.setInt16(
      44 + i * 2,
      Math.floor(Math.sin((2 * Math.PI * 440 * i) / sr) * 16_000),
      true,
    );
  }
  return buf;
...
describe("Audio WM UI — switchAwmTab", () => {
  it("switches to embed tab", () => {
    ...
    globalThis.document.querySelector = (sel) => {
      if (sel === '.tab-btn[data-awm-tab="embed"]')
  ...
  it("switches to extract tab", () => {
    ...
    globalThis.document.querySelector = (sel) => {
      if (sel === '.tab-btn[data-awm-tab="extract"]')
        return { classList: { add() {} } };
      return null;
    };
...
describe("Audio WM UI — toggleAwmPassword", () => {
  it("shows strength group for types 5, 6, 8", () => {
    ...
    assert.notEqual(
      document.getElementById("awm-strength-group").style.display,
      "none",
    );
  });
...
describe("Audio WM UI — toggleAwmPasswordEx", () => {
  it("shows strength group for type 0", () => {
    ...
    assert.notEqual(
      document.getElementById("awm-strength-ex-group").style.display,
      "none",
    );
  });
...
describe("Audio WM UI — toggleAwmInput", () => {
  it("shows text group for low-capacity types (3,4,7)", () => {
    ...
    assert.equal(
      document.getElementById("awm-file-group").style.display,
      "none",
    );
    assert.notEqual(
      document.getElementById("awm-text-group").style.display,
      "none",
    );
  });
...
describe("Audio WM UI — updateAwmCapacity", () => {
  ...
  it("shows capacity with text for low-capacity type 3", () => {
    ...
    globalThis.updateAwmCapacity();
    assert.ok(
      document.getElementById("awm-capacity").textContent.includes("Echo"),
    );
  });
  ...
  it("shows capacity with text for low-capacity type 4", () => {
    ...
    globalThis.updateAwmCapacity();
    assert.ok(
      document.getElementById("awm-capacity").textContent.includes("DSSS"),
    );
  });
  ...
  it("shows capacity with text for low-capacity type 7", () => {
    ...
    globalThis.updateAwmCapacity();
    assert.ok(
      document.getElementById("awm-capacity").textContent.includes("Patchwork"),
    );
  });
});
        ...
        files: [
          {
            ...
            size: 5,
            arrayBuffer: () =>
              Promise.resolve(new Uint8Array([104, 101, 108, 108, 111]).buffer),
...
describe("Audio WM UI — awmSelfTest", () => {
  ...
  it(
    "handles self-test failure gracefully when pw_key throws",
    { timeout: 60_000 },
    async () => {
      resetEls();
      makeEl("awm-output");
      ...
      globalThis.pw_key = async () => {
        throw new Error("mock key error");
      };
      await globalThis.awmSelfTest();
  ...
  it(
    "handles algorithm error during self-test",
    { timeout: 60_000 },
    async () => {
      resetEls();
      makeEl("awm-output");
      ...
      const output = document.getElementById("awm-output");
      assert.ok(
        output.innerHTML.length > 0,
        "Should show results despite algorithm error",
      );
    },
  );

  it(
    "handles dual-watermark payload too large (skip branch)",
    { timeout: 60_000 },
    async () => {
      resetEls();
      makeEl("awm-output");
      ...
      assert.ok(
        output.innerHTML.length > 0,
        "Should show results despite skip",
      );
    },
  );

  it(
    "handles extraction failure during self-test",
    { timeout: 60_000 },
    async () => {
      resetEls();
      makeEl("awm-output");
      ...
      assert.ok(
        output.innerHTML.length > 0,
        "Should show results despite extraction failure",
      );
    },
  );
  ...
  it("handles dual-watermark embed error", { timeout: 60_000 }, async () => {
    ...
    globalThis.aw8_embed_async = () => {
      throw new Error("mock dual embed error");
    };
    await globalThis.awmSelfTest();
    ...
    assert.ok(
      output.innerHTML.length > 0,
      "Should show results despite dual embed error",
    );
  });

  it(
    "handles dual-watermark partial failure (FP extract returns null)",
    { timeout: 120_000 },
    async () => {
      resetEls();
      makeEl("awm-output");
      ...
      assert.ok(
        output.innerHTML.length > 0,
        "Should show results despite dual FP extraction failure",
      );
    },
  );
});
...
describe("Audio WM UI — escapeHtml", () => {
  it("should escape HTML special characters", () => {
    ...
    assert.ok(
      result.includes("&lt;") || result.includes("&amp;") || result.length > 0,
    );
  });
...
describe("Audio WM UI — handleAwmEmbed paths", () => {
  ...
  function setupEmbedMocks(secretBytes) {
    ...
    globalThis._awmSecretBytes =
      secretBytes || new Uint8Array([72, 101, 108, 108, 111]);
    globalThis.validateFileInput = async () => true;
  }

  it(
    "should embed using LSB and show success result",
    { timeout: 30_000 },
    async () => {
      setupEmbedMocks();
      await globalThis.handleAwmEmbed();
      ...
      assert.ok(
        output.innerHTML.length > 0,
        "Output should contain result HTML",
      );
...
describe("Audio WM UI — handleAwmExtract paths", () => {
  ...
  it(
    "should extract LSB watermark and recover message",
    { timeout: 30_000 },
    async () => {
      ...
      const output = document.getElementById("awm-output");
      assert.ok(
        output.innerHTML.includes("SECRET_MSG") ||
          output.innerHTML.includes("extracted") ||
          output.innerHTML.includes("success"),
        "Output should contain extracted message. Got: " +
          output.innerHTML.substring(0, 200),
      );
    },
  );

  it(
    "should show wrong password message for bad password",
    { timeout: 30_000 },
    async () => {
      ...
      assert.ok(
        text.includes("wrong") ||
          ...
          text.includes("no watermark"),
        "Should indicate bad password. Got: " +
          output.innerHTML.substring(0, 200),
      );
    },
  );
  ...
  it(
    "should show no watermark message for clean audio",
    { timeout: 30_000 },
    async () => {
      ...
      assert.ok(
        text.includes("no watermark") ||
          ...
          text.includes("wrong") ||
          text.length > 0,
        "Should indicate no watermark. Got: " +
          output.innerHTML.substring(0, 200),
      );
    },
  );
  ...
  it(
    "should handle extract error gracefully",
    { timeout: 30_000 },
    async () => {
      const file = {
        name: "bad.wav",
...
async function makeWatermarkedAudio(algo, numSamples, password, message) {
  ...
  switch (algo) {
    case 1:
      modified = globalThis.aw1_embed(new Int16Array(s16), payload);
      break;
    case 2:
      modified = globalThis.aw2_embed(new Int16Array(s16), payload, sr);
      break;
    case 3:
      modified = globalThis.aw3_embed(new Int16Array(s16), payload, sr);
      break;
    case 4:
      modified = globalThis.aw4_embed(new Int16Array(s16), payload, sr);
      break;
    case 5:
      modified = globalThis.aw5_embed(new Int16Array(s16), payload, sr);
      break;
    case 6:
      modified = globalThis.aw6_embed(new Int16Array(s16), payload, sr);
      break;
    case 7:
      modified = globalThis.aw7_embed(new Int16Array(s16), payload, sr);
      break;
    case 8:
      modified = await globalThis.aw8_embed_async(
        new Int16Array(s16),
        payload,
        sr,
      );
  ...
  return {
    wavBuf: globalThis.awWriteWav(modified, sr, 1, info.raw, 16),
    s16: modified,
  };
}
...
describe("Audio WM UI — handleAwmEmbed remaining error paths", () => {
  ...
  it("should alert when high capacity has no secret bytes", async () => {
    ...
    assert.ok(
      alerted,
      "should have alerted when no secret bytes for high capacity",
    );
  });

  it(
    "should show message too long error when payload exceeds capacity",
    { timeout: 30_000 },
    async () => {
      ...
      const output = document.getElementById("awm-output");
      assert.ok(
        output.innerHTML.toLowerCase().includes("too long") ||
          output.innerHTML.length > 0,
        "Should show message too long error. Got: " +
          output.innerHTML.substring(0, 200),
      );
    },
  );
...
describe("Audio WM UI — handleAwmEmbed algorithm success", () => {
  it("should embed using FFT-QIM (type 2)", { timeout: 60_000 }, async () => {
    ...
    const output = document.getElementById("awm-output");
    assert.ok(
      output.innerHTML.includes("success") || output.innerHTML.includes("FFT"),
  ...
  it("should embed using DSSS (type 4)", { timeout: 60_000 }, async () => {
    ...
    const output = document.getElementById("awm-output");
    assert.ok(
      output.innerHTML.includes("success") || output.innerHTML.includes("DSSS"),
  ...
  it("should embed using QIM (type 5)", { timeout: 60_000 }, async () => {
    ...
    const output = document.getElementById("awm-output");
    assert.ok(
      output.innerHTML.includes("success") || output.innerHTML.includes("QIM"),
  ...
  it("should embed using DWT (type 6)", { timeout: 60_000 }, async () => {
    ...
    const output = document.getElementById("awm-output");
    assert.ok(
      output.innerHTML.includes("success") || output.innerHTML.includes("DWT"),
  ...
  it("should embed using Patchwork (type 7)", { timeout: 60_000 }, async () => {
    ...
    assert.ok(
      output.innerHTML.includes("success") ||
        output.innerHTML.includes("Patchwork"),
      "Type 7 should succeed. Got: " + output.innerHTML.substring(0, 200),
    );
  });

  it(
    "should embed using DCT async (type 8)",
    { timeout: 120_000 },
    async () => {
  ...
  it(
    "should embed using Echo Hiding (type 3) via handleAwmEmbed",
    { timeout: 120_000 },
    async () => {
      ...
      assert.ok(
        output.innerHTML.includes("success") ||
          output.innerHTML.includes("Echo"),
        "Type 3 embed should succeed. Got: " +
          output.innerHTML.substring(0, 200),
      );
    },
  );
...
describe("Audio WM UI — handleAwmExtract algorithm paths", () => {
  ...
  it(
    "should extract FFT-QWM (type 2) and recover message",
    { timeout: 120_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        2,
        600000,
        "testpass",
        "SECRET_TYPE2",
  ...
  it(
    "should extract DCT async (type 8) and recover message",
    { timeout: 120_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        8,
  ...
  it(
    "should show wrong password message for type 2 with bad password",
    { timeout: 120_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        2,
  ...
  it(
    "should extract QIM (type 5) via handleAwmExtract inline switch",
    { timeout: 60_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        5,
  ...
  it(
    "should extract DWT (type 6) via handleAwmExtract inline switch",
    { timeout: 120_000 },
    async () => {
  ...
  it(
    "should extract Patchwork (type 7) via handleAwmExtract inline switch",
    { timeout: 120_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        7,
  ...
  it(
    "should extract DSSS (type 4) via handleAwmExtract inline switch",
    { timeout: 120_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        4,
  ...
  it(
    "should extract Echo Hiding (type 3) via handleAwmExtract inline switch",
    { timeout: 120_000 },
    async () => {
  ...
  it(
    "should show no watermark message for type 2 on clean audio",
    { timeout: 60_000 },
    async () => {
...
describe("Audio WM UI — handleAwmExtract auto-detect", () => {
  ...
  it(
    "should auto-detect watermark in LSB watermarked audio",
    { timeout: 120_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        1,
  ...
  it(
    "should show no watermark message for clean audio with auto-detect",
    { timeout: 120_000 },
    async () => {
      ...
      assert.ok(
        text.includes("no watermark") ||
          text.includes("not found") ||
          text.length > 0,
        "Auto-detect on clean audio should show appropriate message. Got: " +
...
describe("Audio WM UI — awmMultiDetect", () => {
  it("should return false for clean audio", { timeout: 120_000 }, async () => {
    ...
    const result = await globalThis.awmMultiDetect(
      info,
      key,
      spinner,
      prog,
  ...
  it(
    "should detect watermark in LSB watermarked audio",
    { timeout: 120_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        1,
...
describe("Audio WM UI — awmDualExtract", () => {
  ...
  it(
    "should handle dual extract with auto-detect algo 0 on watermarked audio",
    { timeout: 60_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        1,
      ...
      assert.ok(
        output.innerHTML.length > 0,
        "Dual extract auto-detect should produce output",
      );
    },
  );

  it(
    "should handle dual extract with watermarked mono audio (LSB)",
    { timeout: 60_000 },
    async () => {
      const wm = await makeWatermarkedAudio(
        1,
      ...
      const output = document.getElementById("awm-output");
      assert.ok(
        output.innerHTML.includes("DUAL_WM_DATA") ||
          output.innerHTML.includes("Fingerprint"),
        "Dual extract should find watermark. Got: " +
          output.innerHTML.substring(0, 200),
      );
    },
  );
  ...
  it(
    "should handle dual extract error gracefully",
    { timeout: 10_000 },
    async () => {
      const file = {
        name: "bad.wav",
      ...
      assert.ok(
        output.innerHTML.length > 0,
        "Dual extract error should produce output",
      );
    },
  );

  it(
    "should handle dual extract gracefully with clean mono audio",
    { timeout: 60_000 },
    async () => {
      resetEls();
      const cleanBuf = makeTestWav(5000, 44100);
      ...
      assert.ok(
        output.innerHTML.length > 0,
        "Dual extract should produce output. Got: " +
          output.innerHTML.substring(0, 200),
      );
    },
  );
...
describe("Audio WM UI — tryExtractSingle algorithm paths", () => {
  it(
    "should extract DSSS (type 4) message via tryExtractSingle",
    { timeout: 120_000 },
    async () => {
      const sr = 44100;
      ...
      assert.ok(
        ...
        "tryExtractSingle type 4 should return decoded message",
      );
      assert.ok(
        result.decoded.includes("TYPE4_DSSS"),
        "Should recover 'TYPE4_DSSS'. Got: " +
          (result ? result.decoded : "null"),
      );
    },
  );

  it(
    "should extract QIM (type 5) message via tryExtractSingle",
    { timeout: 60_000 },
    async () => {
      const sr = 44100;
      const numSamples = 5000;
      ...
      assert.ok(
        result && result.decoded,
        "tryExtractSingle type 5 should return decoded message",
      );
      assert.ok(
        result.decoded.includes("TYPE5_QIM"),
        "Should recover 'TYPE5_QIM'. Got: " +
          (result ? result.decoded : "null"),
      );
    },
  );
  ...
  it(
    "should extract Patchwork (type 7) message via tryExtractSingle",
    { timeout: 120_000 },
    async () => {
      const sr = 44100;
      ...
      assert.ok(
        result && result.decoded,
        "tryExtractSingle type 7 should return decoded message",
      );
      assert.ok(
        result.decoded.includes("TYPE7_PATCH"),
        "Should recover 'TYPE7_PATCH'. Got: " +
          (result ? result.decoded : "null"),
      );
    },
  );
  ...
  it(
    "should extract Echo Hiding (type 3) message via tryExtractSingle",
    { timeout: 120_000 },
    async () => {
      const sr = 44100;
      ...
      assert.ok(
        result && result.decoded,
        "tryExtractSingle type 3 should return decoded message",
      );
      assert.ok(
        result.decoded.includes("Hi"),
  ...
  it(
    "should return null for tryExtractSingle with wrong password",
    { timeout: 60_000 },
    async () => {
      const sr = 44100;
      const numSamples = 5000;
      ...
      assert.equal(
        result,
        null,
        "tryExtractSingle with wrong password should return null",
      );
    },
  );
});
...
describe("Audio WM UI — stereo branch coverage", () => {
  ...
  it(
    "should handle stereo audio in awmMultiDetect right channel branch",
    { timeout: 120_000 },
    async () => {
      const sr = 44100;
      const numSamples = 5000;
      ...
      const key = await globalThis.pw_key("testpass");
      const payload = globalThis.awFormatPayload(
        new TextEncoder().encode("STEREO_TEST"),
        key,
      );
      ...
      const wmInfo = globalThis.awReadWavRaw(
        new Uint8Array(stereoWmBuf).buffer,
      );
      ...
      assert.equal(
        result,
        true,
        "awmMultiDetect should find watermark in stereo audio",
      );
    },
  );

  it(
    "should handle stereo audio in awmDualExtract right channel branch",
    { timeout: 120_000 },
    async () => {
      const sr = 44100;
      const numSamples = 5000;
      ...
      const key = await globalThis.pw_key("testpass");
      const payload = globalThis.awFormatPayload(
        new TextEncoder().encode("DUAL_STEREO"),
        key,
      );
      globalThis.aw1_embed(s16, payload);
      const stereoWmBuf = globalThis.awWriteWav([s16, s16], sr, 2, null, 16);
      ...
      const output = document.getElementById("awm-output");
      assert.ok(
        output.innerHTML.includes("DUAL_STEREO") ||
          output.innerHTML.includes("Fingerprint"),
        "Stereo dual extract should find watermark. Got: " +
          output.innerHTML.substring(0, 200),
      );
    },
  );
