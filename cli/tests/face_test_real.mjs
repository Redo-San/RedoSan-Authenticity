...
page.on("console", (msg) => {
  const text = msg.text();
  if (msg.type() === "error" || /error|fail/i.test(text)) errors.push(msg.type() + ": " + text);
});
...
console.log("CONSOLE MESSAGES (headless WebGL fallbacks expected): " + JSON.stringify(errors, null, 2));
console.log("RESULT arcface=" + (arcOk ? "PASS" : "FAIL") + " human=" + (humOk ? "PASS" : "FAIL"));
