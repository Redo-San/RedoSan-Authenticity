...
if (typeof globalThis.crypto === 'undefined' || !globalThis.crypto.subtle) {
  globalThis.crypto = {
    subtle: {
      digest: async (algo, data) => { const n = typeof algo === 'string' ? algo : algo.name || 'SHA-256'; const h = crypto.createHash(n.toLowerCase().replace('-', '')).update(Buffer.from(data)).digest(); return h.buffer; },
      importKey: async (f, kd, algo, ext, us) => ({ type: 'secret', algorithm: algo, keyData: kd }),
      deriveBits: async (algo, baseKey, len) => { const pw = Buffer.from(baseKey.keyData); const s = algo.salt || pw; const it = algo.iterations || 100000; const h = typeof algo.hash === 'string' ? algo.hash.replace('-', '').toLowerCase() : 'sha256'; const d = crypto.pbkdf2Sync(pw, s, it, len / 8, h); return d.buffer; }
    }
  };
}
if (typeof globalThis.window === 'undefined') globalThis.window = globalThis;
if (typeof globalThis.sha256Hex === 'undefined') {
  globalThis.sha256Hex = async function(data) { const h = crypto.createHash('sha256').update(Buffer.from(data)).digest(); return Array.from(h).map(b => b.toString(16).padStart(2, '0')).join(''); };
}
const vm = require('vm');
...
try { watermarkCore = new globalThis.WatermarkCore(); } catch(e) {}
...
if (fs.existsSync(audioCorePath)) {
  ...
  try { vm.runInThisContext(src, { filename: 'audio_watermark_core.js' }); audioCoreLoaded = true; } catch(e) { console.warn('Warning: Could not load audio_watermark_core.js:', e.message); }
}
...
function cleanLSB(imgData, bits) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    d[i] &= mask; d[i+1] &= mask; d[i+2] &= mask;
  }
}
...
(async function() {
  ...
  if (watermarkCore) {
    ...
    function piExtract(img, algo, pw) {
      try { return watermarkCore.detection.blind_decoding(img, algo, pw || ''); } catch(e) { return ''; }
    }
    ...
    test('PI enhanced_lsb: embed → extract succeeds', () => {
      ...
      assert(result && result.length > 0 && result.indexOf(testMsg) >= 0, 'Should extract original message after embed');
    });
  ...
  if (audioCoreLoaded) {
    ...
    function createTestAudio(sr, durationSec) {
      ...
      for (let i = 0; i < numSamples; i++) {
        samples[i] = Math.sin(2 * Math.PI * 440 * i / sr) * 16000 +
