...
global.ImageData =
  global.ImageData || (typeof ImageData === "undefined" ? null : ImageData);
global.requestAnimationFrame = (cb) => setTimeout(cb, 0);
global.crypto = global.crypto || require("node:crypto").webcrypto;
global.__ = (k, d) => (d === undefined ? k : d);

const files = [
  "Iris_Biometric/iris_quality_full.js",
  "Iris_Biometric/iris_ui.js",
];
...
test("generateQualityVector: specular in slot 18 (idx 17), VIA in slot 19 (idx 18)", () => {
  ...
  assert.ok(
    v[17] > 0,
    "specular vendor slot 18 (idx 17) should be > 0, got " + v[17],
  );
  assert.ok(
    v[11] > 0,
    "defined specular metric slot 12 (idx 11) should be > 0, got " + v[11],
  );
  assert.ok(
    v[18] > 0 && v[18] <= 100,
    "VIA slot 19 (idx 18) should be in (0,100], got " + v[18],
  );
});
