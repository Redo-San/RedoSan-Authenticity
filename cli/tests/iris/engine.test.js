...
test("IE._toGrayscale: ImageData input (L101-L103)", () => {
  const imgData = new ImageData(new Uint8Array(100 * 100 * 4).fill(128), 100, 100);
...
test("IE.validateEyePresence: no-dark-pupil (L327)", () => {
  ...
  const r = IE.validateEyePresence(gray, 200, 200, { cx: 100, cy: 100, radius: 30 }, { cx: 100, cy: 100, radius: 80 });
...
test("IE.validateEyePresence: low-iris-texture (L328-L330)", () => {
  ...
  for (let y = 0; y < 200; y++) for (let x = 0; x < 200; x++) {
    ...
    if (d < 30) gray[y * 200 + x] = 20; else if (d < 80) gray[y * 200 + x] = 100; else gray[y * 200 + x] = 180;
  }
  const r = IE.validateEyePresence(gray, 200, 200, { cx: 100, cy: 100, radius: 30 }, { cx: 100, cy: 100, radius: 80 });
  assert.equal(r.ok, false);
...
test("IE.validateEyePresence: ok path (L330-L331)", () => {
  ...
  for (let y = 0; y < 200; y++) for (let x = 0; x < 200; x++) {
    const d = Math.hypot(x - 100, y - 100);
    if (d < 30) gray[y * 200 + x] = 20; else if (d < 80) gray[y * 200 + x] = 100 + ((x + y) % 40); else gray[y * 200 + x] = 180;
  }
  const r = IE.validateEyePresence(gray, 200, 200, { cx: 100, cy: 100, radius: 30 }, { cx: 100, cy: 100, radius: 80 });
...
test("IE.normalize: default normW/normH (L448)", () => {
  ...
  const r = IE.normalize(gray, 100, 100, { cx: 50, cy: 50, radius: 30 }, { cx: 50, cy: 50, radius: 45 });
  assert.ok(r);
});
...
test("IE.validateEyePresence: full pass including _meanDisk and _varAnnulus (L340, L429)", () => {
  ...
  const r = IE.validateEyePresence(gray, w, h,
...
test("IE.validateEyePresence: gradient image with dark pupil (L340)", () => {
  ...
  for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
    ...
    else if (d < 110) gray[y * w + x] = 110 + Math.sin(x * 0.15 + y * 0.1) * 25;
    else gray[y * w + x] = 180;
  }
  const r = IE.validateEyePresence(gray, w, h,
    { cx: 150, cy: 150, radius: 25 },
...
test("IE._varAnnulus: gradient in annulus region (L429)", () => {
  const w = 200, h = 200;
  const gray = new Uint8Array(w * h);
  for (let i = 0; i < gray.length; i++) gray[i] = 100 + Math.sin(i * 0.005) * 30;
