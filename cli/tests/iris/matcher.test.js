...
test("IrisMatcher.hammingDistance: opposite codes → hd=1", () => {
  const a = {
    code: new Uint8Array([1, 1, 1, 1]),
    mask: new Uint8Array([1, 1, 1, 1]),
  };
  const b = {
    code: new Uint8Array([0, 0, 0, 0]),
    mask: new Uint8Array([1, 1, 1, 1]),
  };
...
test("IrisMatcher.compare: opposite → no match", () => {
  const a = {
    code: new Uint8Array(16).fill(1),
    mask: new Uint8Array(16).fill(1),
  };
  const b = {
    code: new Uint8Array(16).fill(0),
    mask: new Uint8Array(16).fill(1),
  };
...
test("IrisMatcher.compare: low overlap → 'Low overlap' detail", () => {
  const a = {
    code: new Uint8Array(100).fill(1),
    mask: new Uint8Array(100).fill(0),
  };
  const b = {
    code: new Uint8Array(100).fill(0),
...
test("IrisMatcher.compare: marginal match", () => {
  ...
  const r = IrisMatcher.compare({ code, mask }, { code: bCode, mask }, 0.3);
  assert(r.match);
...
test("IrisMatcher.identify: no match in gallery → bestMatch=null", () => {
  ...
  const r = IrisMatcher.identify(probe, [g], 0.1);
  assert.strictEqual(r.bestMatch, null);
});
...
test("IM.identify: with gallery returns results object (L191-L224)", () => {
  const probe = {
    code: new Uint8Array(100).fill(0xff),
    mask: new Uint8Array(100).fill(1),
  };
  const gallery = [
    {
    ...
    {
      id: "2",
      code: new Uint8Array(100).fill(0xaa),
      mask: new Uint8Array(100).fill(1),
    },
  ];
  const result = IM.identify(probe, gallery);
...
test("IM.identify: with threshold arg", () => {
  const probe = {
    code: new Uint8Array(64).fill(0xff),
  ...
  const gallery = [
    ...
    {
      id: "mismatch",
      code: new Uint8Array(64).fill(0x00),
      mask: new Uint8Array(64).fill(1),
    },
  ];
...
test("IM.compare: default threshold (L146-L151)", () => {
  ...
  c2.fill(0xff);
  const m1 = new Uint8Array(64);
  m1.fill(1);
  const m2 = new Uint8Array(64);
  m2.fill(1);
...
test("IM.compare: Good match tier (L166)", () => {
  ...
  assert.ok(r);
  assert.ok(
    r.details.includes("match") ||
      r.details.includes("Good") ||
      r.details.includes("Excellent") ||
      r.details.includes("Marginal") ||
      r.details.includes("No match"),
  );
});
