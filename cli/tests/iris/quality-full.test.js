...
test("IQF constructor (L26-L27)", () => { const q = new IQF(); assert.ok(q); });
test("IQF.focusQuality: with ROI (L263-L266)", () => {
  ...
  const r = IQF.focusQuality(data, w, h, { x: 20, y: 20, width: 60, height: 60 });
  assert.equal(typeof r, "number");
});
test("IQF.rawLaplacianVariance: with ROI (L333)", () => {
  const w = 10, h = 10;
  const data = new Uint8Array(w * h * 4);
  for (let i = 0; i < data.length; i += 4) { data[i] = 100 + (i % 50); data[i+1] = 100; data[i+2] = 100; data[i+3] = 255; }
  const r = IQF.rawLaplacianVariance(data, w, h, { x: 0, y: 0, width: 10, height: 10 });
  assert.equal(typeof r, "number");
});
...
test("IQF.grayscaleUtilization: with ROI (L816)", () => {
  ...
  const r = IQF.grayscaleUtilization(data, { x: 10, y: 10, width: 30, height: 30 }, 100);
  assert.equal(typeof r, "number");
});
...
test("IQF.pupilBoundaryCircularity: valid mask (L885-L898)", () => {
  const w = 64, h = 64;
  const mask = new Uint8Array(w * h).fill(1);
  for (let y = 20; y < 44; y++) for (let x = 20; x < 44; x++) mask[y * w + x] = 0;
...
test("IQF.motionBlurFocus: valid image (L924-L925)", () => {
  const w = 10, h = 10;
  const data = new Uint8Array(w * h * 4);
  for (let i = 0; i < data.length; i += 4) { data[i] = 100 + (i % 80); data[i+1] = 100 + (i % 60); data[i+2] = 100; data[i+3] = 255; }
...
test("IQF.specularReflection: valid data (L948-L957)", () => {
  ...
  for (let i = 0; i < data.length; i += 4) { data[i] = 128; data[i+1] = 128; data[i+2] = 128; data[i+3] = 255; }
  data[(32 * w + 32) * 4] = 255;
  const r = IQF.specularReflection(data, w, h, { cx: 32, cy: 32, radius: 10 }, { x: 32, y: 32, radius: 25 });
  assert.ok(r);
...
test("IQF.eyelidCircularity: valid mask (L1036)", () => {
  const w = 64, h = 64;
  const mask = new Uint8Array(w * h).fill(1);
  for (let y = 15; y < 49; y++) for (let x = 15; x < 49; x++) mask[y * w + x] = 0;
...
test("IQF.detectNirCapability: NIR device found (L1137-L1141)", async () => {
  ...
  global.navigator.mediaDevices = {
    enumerateDevices: async () => [
      { kind: "videoinput", label: "IR Camera NIR Sensor", getCapabilities: () => ({ facingMode: "environment" }) },
    ],
  };
  const r = await IQF.detectNirCapability();
...
test("IQF.detectNirCapability: no NIR but environment cam (L1142-L1144)", async () => {
  ...
  global.navigator.mediaDevices = {
    enumerateDevices: async () => [
      { kind: "videoinput", label: "Back Camera", getCapabilities: () => ({ facingMode: "environment" }) },
    ],
  };
  const r = await IQF.detectNirCapability();
  assert.equal(r.nirAvailable, false);
...
test("IQF.detectNirCapability: error path", async () => {
  const orig = global.navigator.mediaDevices;
  global.navigator.mediaDevices = {
    enumerateDevices: async () => { throw new Error("denied"); },
  };
  const r = await IQF.detectNirCapability();
  assert.equal(r.nirAvailable, false);
...
test("IQF.generateQualityVector: with iris param (L1175-L1218)", () => {
  ...
  const r = IQF.generateQualityVector({
    imageData: data, width: w, height: h,
...
test("IQF._generateReport: with metrics (L1493)", () => {
  const r = IQF._generateReport(75, { label: "Good", code: 3 }, { focus: 0.85, contrast: 0.7 }, true);
...
test("IQF.computeCompositeQuality: full params (L1200, L1302, L1368, L1388, L1463)", () => {
  ...
  for (let i = 0; i < imageData.length; i++) imageData[i] = 80 + Math.sin(i * 0.1) * 40;
  const mask = new Uint8Array(w * h).fill(1);
  const params = {
    imageData, width: w, height: h, mask,
...
test("IQF.marginAdequacy: iris near bottom → bottom margin (L797)", () => {
  const w = 100, h = 100;
  const r = IQF.marginAdequacy({ x: 50, y: 85, width: 30, height: 30 }, 15, w, h);
  assert.equal(typeof r, "number");
  assert.ok(r >= 0);
});
...
test("IQF.mutualQualityComparison: consistency with different scores (L1302)", () => {
  ...
  for (let i = 0; i < img1.length; i++) { img1[i] = 100 + (i % 50); img2[i] = 150 + (i % 50); }
  const mask = new Uint8Array(w * h).fill(1);
  const params = {
    imageData1: img1, imageData2: img2, width: w, height: h,
    mask, pupil: { x: 25, y: 25, radius: 8 },
    iris: { x: 25, y: 25, radius: 20 },
  };
