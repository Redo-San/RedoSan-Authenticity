...
vm.runInThisContext(
  fs.readFileSync(
    path.join(__dirname, "..", "..", "Iris_Biometric", "iris_quality_full.js"),
    "utf8",
  ),
  {
    filename: path.join(
...
function buildImage(annulusFn) {
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      else if (d >= iris.radius * 0.4 && d <= iris.radius * 0.9)
        v = annulusFn(x, y, d);
...
test("irisTextureContrast: textured annulus has high std; flat annulus ~0", () => {
  const textured = buildImage(
    (x, y) => 128 + 60 * Math.sin(x / 3) * Math.cos(y / 3),
  );
  ...
  assert.ok(
    cTextured > 10,
    "textured iris should exceed the gate floor: " + cTextured.toFixed(2),
  );
  assert.ok(
    cFlat < 1,
    "flat iris annulus should be near-zero texture: " + cFlat.toFixed(2),
  );
});
...
test("evaluateAcquisitionGates: rejects a near-flat (low-texture) iris", () => {
  ...
  assert.ok(
    res.failures.some((f) => f.indexOf("irisTextureContrast") === 0),
...
test("evaluateAcquisitionGates: accepts a well-textured iris", () => {
  ...
  const img = buildImage(
    (x, y) => 128 + 60 * Math.sin(x * 2.5) * Math.sin(y * 2.5),
  );
  const res = Q.evaluateAcquisitionGates({
    imageData: img,
    width: W,
  ...
  assert.strictEqual(
    res.passed,
    true,
    "unexpected failures: " + JSON.stringify(res.failures),
  );
});
