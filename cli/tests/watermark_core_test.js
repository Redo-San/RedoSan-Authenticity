...
try {
  ...
  // canvas native module not available — tests will skip below
}

if (!createCanvas) {
  const { test } = require("node:test");
  test("skip — canvas native module not available", (t) => t.skip());
  module.exports = {};
} else {
  // Polyfills
  globalThis.document = {
  ...
  if (typeof globalThis.crypto === "undefined" || !globalThis.crypto.subtle) {
    globalThis.crypto = {
      subtle: {
        digest: async (_algo, data) => crypto.createHash("sha256").update(Buffer.from(data)).digest(),
        importKey: async (_f, kd) => ({ type: "secret", keyData: kd }),
