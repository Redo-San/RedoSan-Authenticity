...
const src = fs.readFileSync(
  path.join(__dirname, "../../Metadata/metadata.js"),
  "utf8",
);
vm.runInThisContext(src, {
  filename: path.resolve(__dirname, "../../Metadata/metadata.js"),
});
...
describe("Metadata — parseJPEGExif", () => {
  ...
  it("should return empty object for non-JPEG data", () => {
    const data = new Uint8Array([
      0xff, 0xd8, 0x89, 0x50, 0x4e, 0x47, 0xff, 0xd9,
    ]);
  ...
  it("should handle JPEG with APP1 but no Exif header", () => {
    const arr = [
      0xff, 0xd8, 0xff, 0xe1, 0x00, 0x08, 0x4e, 0x6f, 0x74, 0x45, 0x78, 0x69,
      0x66, 0x00, 0xff, 0xd9,
    ];
...
const JPEG_TYPE5 = new Uint8Array([
  ...
  0x00,
  0x00,
  0x00, // IFD0 TIFF offset = 8
  0x01,
  0x00, // 1 entry
  0x9d,
  0x82, // tag 0x829D (FNumber)
  0x05,
  0x00, // type 5 (RATIONAL)
  0x01,
  0x00,
  0x00,
  0x00, // count = 1
...
const JPEG_INLINE_TYPE2 = new Uint8Array([
  ...
  0x02,
  0x00, // type 2 (ASCII) with count <= 4 → inline in value field
  0x04,
  0x00,
  0x00,
  0x00, // count = 4
  ...
  0x42,
  0x43,
  0x00, // inline "ABC\0" in 4-byte value field
  0x00,
  0x00,
  0x00,
  0x00, // next IFD = 0
...
const JPEG_TYPE7 = new Uint8Array([
  ...
  0x00, // 1 entry
  0xab,
  0x89, // tag 0x89AB (not in EXIF_TAGS)
  0x07,
...
function makeJPEGWithMakeValue(str) {
  ...
  w(0xe1); // APP1
  w16be(segLen);
  data.set([0x45, 0x78, 0x69, 0x66, 0x00, 0x00], off);
  off += 6; // "Exif\0\0"
  w(0x49);
  w(0x49); // LE endian
  w(0x2a);
  ...
  w32le(0); // next IFD
  data.set(valueBytes, off);
  off += count; // string value
  // SOS + EOI
  data.set(
...
describe("Metadata — parseIFD big-endian TIFF", () => {
  it("should parse EXIF with big-endian byte order (MM)", () => {
    ...
    const jpeg = new Uint8Array([
      ...
      0x00, // Exif header
      0x4d,
      0x4d, // endian (MM = big endian)
      0x00,
      0x2a, // magic 42 (big-endian order)
      0x00,
      0x00,
      0x00,
      0x08, // IFD0 offset = 8 (big-endian)
      0x00,
      0x01, // 1 entry (big-endian)
      0x01,
      0x0f, // tag 0x010F Make (big-endian)
      0x00,
      0x02, // type 2 ASCII (big-endian)
      0x00,
      0x00,
      0x00,
      0x0a, // count = 10 (big-endian)
      0x00,
      0x00,
      0x00,
      0x1a, // value TIFF offset = 26 (big-endian)
      0x00,
      0x00,
      0x00,
      0x00, // next IFD = 0
