...
function exifRead(filePath) {
  ...
  if (!tool)
    return {
      from: "js",
      warning: "exiftool not installed — JS EXIF limited to JPEG",
    };
...
function audioConvert(input, output) {
  ...
  if (!tool) {
    ...
    return {
      ok: false,
      warning: "sox not installed — file copied without conversion",
    };
  }
...
function imageConvert(input, output) {
  ...
  if (!tool) {
    ...
    return {
      ok: false,
      warning: "ImageMagick not installed — file copied without conversion",
    };
  }
  const out = _runTool(tool, [input, output]);
  if (out.error) return { ok: false, error: out.error };
