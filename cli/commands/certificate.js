...
async function buildCertData(filePath, opts) {
  ...
  if (filePath && fs.existsSync(filePath)) {
    ...
    const mimeMap = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.bmp': 'image/bmp', '.tiff': 'image/tiff', '.tif': 'image/tiff', '.webp': 'image/webp' };
...
function generatePDF(data, qrPngBuf, qrContent) {
  ...
  const margin = 50, pageW = pw - 2 * margin;
  let y = margin;
  ...
  if (data.user.name) {
    checkPage(14);
    doc.fontSize(14).font('Helvetica-Bold').text('Owner', margin, y); y += 6;
    doc.fontSize(10).font('Helvetica');
    if (data.user.name) { doc.font('Helvetica-Bold').text('Name: ', margin, y, { continued: true }); doc.font('Helvetica').text(data.user.name); y += 5; }
    if (data.user.email) { doc.font('Helvetica-Bold').text('Email: ', margin, y, { continued: true }); doc.font('Helvetica').text(data.user.email); y += 5; }
    if (data.user.phone) { doc.font('Helvetica-Bold').text('Phone: ', margin, y, { continued: true }); doc.font('Helvetica').text(data.user.phone); y += 5; }
    if (data.user.website) { doc.font('Helvetica-Bold').text('Website: ', margin, y, { continued: true }); doc.font('Helvetica').text(data.user.website); y += 5; }
    y += 3;
  }
  ...
  if (socialEntries.length > 0) {
    ...
    for (const [k, v] of socialEntries) {
      doc.text(k.charAt(0).toUpperCase() + k.slice(1) + ': ' + v, margin, y); y += 5;
    }
    y += 2;
  }
  ...
  // 2. File Info
  checkPage(18);
  doc.fontSize(14).font('Helvetica-Bold').text('File Information', margin, y); y += 6;
  doc.fontSize(10).font('Helvetica');
  if (data.file.name) { doc.font('Helvetica-Bold').text('Name: ', margin, y, { continued: true }); doc.font('Helvetica').text(data.file.name); y += 5; }
  if (data.file.size) { doc.font('Helvetica-Bold').text('Size: ', margin, y, { continued: true }); doc.font('Helvetica').text(fmtSize(data.file.size)); y += 5; }
  if (data.file.width) { doc.font('Helvetica-Bold').text('Dimensions: ', margin, y, { continued: true }); doc.font('Helvetica').text(data.file.width + ' x ' + data.file.height + ' px'); y += 5; }
  if (data.file.hash) { doc.font('Helvetica-Bold').text('SHA-256: ', margin, y, { continued: true }); doc.font('Helvetica').text(data.file.hash); y += 5; }
  y += 2;

  // Embed image
  if (data.file.buf) {
    const imgMaxW = pageW, imgMaxH = 240;
    let imgW = data.file.width || 400, imgH = data.file.height || 300;
    if (imgW > imgMaxW) { imgH = imgH * imgMaxW / imgW; imgW = imgMaxW; }
    if (imgH > imgMaxH) { imgW = imgW * imgMaxH / imgH; imgH = imgMaxH; }
    checkPage(imgH + 10);
    try {
  ...
  if (data.watermark) {
    checkPage(12);
    doc.fontSize(14).font('Helvetica-Bold').text('Watermark', margin, y); y += 6;
    doc.fontSize(10).font('Helvetica');
    doc.font('Helvetica-Bold').text('Result: ', margin, y, { continued: true }); doc.font('Helvetica').text(data.watermarkAlgo || 'Completed'); y += 5;
  ...
  if (data.pixelInjection) {
    checkPage(12);
    doc.fontSize(14).font('Helvetica-Bold').text('Pixel Injection', margin, y); y += 6;
    doc.fontSize(10).font('Helvetica');
    doc.font('Helvetica-Bold').text('Result: ', margin, y, { continued: true }); doc.font('Helvetica').text('Completed'); y += 5;
  ...
  if (data.timestamp) {
    checkPage(12);
    doc.fontSize(14).font('Helvetica-Bold').text('Timestamp', margin, y); y += 6;
    doc.fontSize(10).font('Helvetica');
    if (data.tsResult) {
      doc.fontSize(8).font('Helvetica');
      ...
      doc.text('Timestamp created successfully.', margin, y); y += 5;
    }
    y += 2;
  }
  ...
  if (data.fingerprint && data.fpResult && data.fpResult.hashes) {
    checkPage(16);
    doc.fontSize(14).font('Helvetica-Bold').text('Fingerprint (Hashes)', margin, y); y += 6;
    ...
    for (const fam of families) {
      ...
      doc.font('Helvetica-Bold').fontSize(9).text(fam.label, margin, y); y += 4;
      doc.font('Courier').fontSize(7);
      for (const k of fam.keys) {
        const v = data.fpResult.hashes[k];
        if (v) { doc.text(k + ':  ' + v, margin, y); y += 3.5; }
      }
      y += 1;
    }
    if (data.fpResult.perceptual_hashes) {
      for (const [pk, pv] of Object.entries(data.fpResult.perceptual_hashes)) {
        checkPage(6);
        doc.font('Helvetica-Bold').fontSize(9).text(pk, margin, y); y += 4;
        doc.font('Courier').fontSize(7).text(pv, margin, y); y += 4;
      }
    }
  }
  ...
  if (data.didSig && data.didSig.did) {
    checkPage(20);
    doc.fontSize(14).font('Helvetica-Bold').text('Decentralized Identity (DID)', margin, y); y += 6;
    doc.fontSize(8).font('Courier');
    doc.text('DID: ' + data.didSig.did, margin, y); y += 4;
    doc.text('Algorithm: ' + (data.didSig.algorithm || 'Ed25519'), margin, y); y += 4;
    doc.text('Signed: ' + (data.didSig.timestamp || '').replace('T', ' ').substring(0, 19), margin, y); y += 4;
    doc.fontSize(7);
    ...
    checkPage(10);
    doc.fontSize(14).font('Helvetica-Bold').text('DID Identity', margin, y); y += 6;
    doc.fontSize(8).font('Courier');
  ...
  if (data.ct && data.ct.submitted && data.ct.hash) {
    checkPage(12);
    doc.fontSize(12).font('Helvetica-Bold').text('Certificate Transparency', margin, y); y += 5;
  ...
  doc.text('Verification QR Code', pw / 2, y, { align: 'center' }); y += 7;
  doc.fontSize(8).font('Helvetica');
  doc.text('Scan this QR code to verify the document contents. The QR encodes all', pw / 2, y, { align: 'center' }); y += 4;
  doc.text('verification data (hashes, file info, owner). Any mismatch indicates tampering.', pw / 2, y, { align: 'center' }); y += 8;
...
async function generateDOCX(data, qrPngBuf, qrContent) {
  let docx;
  try {
    docx = require('docx');
  } catch (e) {
...
async function generateEPUB(data, qrPngBuf, qrContent) {
  ...
  if (data.user.name) {
    ...
    for (const [k, v] of Object.entries(data.user.social)) {
      if (v) userSection += '<tr><td><strong>' + escHtml(k.charAt(0).toUpperCase() + k.slice(1)) + '</strong></td><td>' + escHtml(v) + '</td></tr>';
    }
    userSection += '</table>';
  }
  ...
  if (data.fingerprint && data.fpResult && data.fpResult.hashes) {
    ...
    for (const fam of families) {
      ...
      for (const k of fam.keys) {
        const v = data.fpResult.hashes[k];
        if (v) fpSection += '<tr><td><strong>' + escHtml(k) + '</strong></td><td style="font-size:0.7em;word-break:break-all">' + escHtml(v) + '</td></tr>';
      }
      fpSection += '</table>';
    }
    if (data.fpResult.perceptual_hashes) {
  ...
  const css = 'body{font-family:serif;padding:20px;max-width:800px;margin:0 auto}' +
