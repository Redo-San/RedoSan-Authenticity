...
async function buildCertData(filePath, opts) {
  ...
  if (opts.timestamp && fs.existsSync(opts.timestamp)) {
    ...
    data.tsResult = `Timestamp file: ${path.basename(
      opts.timestamp,
    )} (${fmtSize(tsStat.size)})`;
  }
...
function generatePDF(data, qrPngBuf, qrContent) {
  return new Promise((resolve, reject) => {
    ...
    doc.text(
      `Generated by RedoSan Authenticity — ${data.generatedAt
        .replace("T", " ")
        .substring(0, 19)}`,
      pw / 2,
      y,
    ...
    if (data.user.name) {
      ...
      if (data.user.name) {
        doc
          .font("Helvetica-Bold")
          .text("Name: ", margin, y, { continued: true });
      ...
      if (data.user.email) {
        doc
          .font("Helvetica-Bold")
          .text("Email: ", margin, y, { continued: true });
      ...
      if (data.user.phone) {
        doc
          .font("Helvetica-Bold")
          .text("Phone: ", margin, y, { continued: true });
      ...
      if (data.user.website) {
        doc
          .font("Helvetica-Bold")
          .text("Website: ", margin, y, { continued: true });
    ...
    if (data.file.width) {
      doc
        .font("Helvetica-Bold")
        .text("Dimensions: ", margin, y, { continued: true });
    ...
    if (data.file.hash) {
      doc
        .font("Helvetica-Bold")
        .text("SHA-256: ", margin, y, { continued: true });
    ...
    if (data.watermark) {
      ...
      doc
        .font("Helvetica-Bold")
        .text("Result: ", margin, y, { continued: true });
    ...
    if (data.fingerprint && data.fpResult?.hashes) {
      ...
      doc
        .fontSize(14)
        .font("Helvetica-Bold")
        .text("Fingerprint (Hashes)", margin, y);
      y += 6;
      ...
      if (data.fpResult.perceptual_hashes) {
        for (const [pk, pv] of Object.entries(
          data.fpResult.perceptual_hashes,
        )) {
          checkPage(6);
    ...
    if (data.didSig?.did) {
      ...
      doc
        .fontSize(14)
        .font("Helvetica-Bold")
        .text("Decentralized Identity (DID)", margin, y);
      y += 6;
      doc.fontSize(8).font("Courier");
    ...
    if (data.ct?.submitted && data.ct.hash) {
      ...
      doc
        .fontSize(12)
        .font("Helvetica-Bold")
        .text("Certificate Transparency", margin, y);
      y += 5;
    ...
    y += 4;
    doc.text(
      "verification data (hashes, file info, owner). Any mismatch indicates tampering.",
      pw / 2,
      y,
      {
        align: "center",
...
async function generateDOCX(data, qrPngBuf, _qrContent) {
  ...
  if (data.file.width)
    addLabelValue("Dimensions", `${data.file.width} x ${data.file.height} px`);
...
async function generateEPUB(data, qrPngBuf, qrContent) {
  ...
  if (data.user.name) {
    ...
    if (data.user.name)
      userSection += `<tr><td><strong>Name</strong></td><td>${escHtml(
        data.user.name,
      )}</td></tr>`;
    if (data.user.email)
      userSection += `<tr><td><strong>Email</strong></td><td>${escHtml(
        data.user.email,
      )}</td></tr>`;
    if (data.user.phone)
      userSection += `<tr><td><strong>Phone</strong></td><td>${escHtml(
        data.user.phone,
      )}</td></tr>`;
    if (data.user.website)
      userSection += `<tr><td><strong>Website</strong></td><td>${escHtml(
        data.user.website,
      )}</td></tr>`;
    for (const [k, v] of Object.entries(data.user.social)) {
      if (v)
  ...
  if (data.fingerprint && data.fpResult?.hashes) {
      ...
      for (const k of fam.keys) {
        ...
        if (v)
          fpSection += `<tr><td><strong>${escHtml(
            k,
          )}</strong></td><td style="font-size:0.7em;word-break:break-all">${escHtml(
            v,
          )}</td></tr>`;
      }
      fpSection += "</table>";
    ...
    if (data.fpResult.perceptual_hashes) {
      for (const [pk, pv] of Object.entries(data.fpResult.perceptual_hashes)) {
        fpSection += `<h3>${escHtml(
          pk,
        )}</h3><p style="font-size:0.7em;word-break:break-all">${escHtml(
          pv,
        )}</p>`;
      }
    }
  ...
  const xhtml =
    ...
    (data.pixelInjection
      ...
      : "") +
    (data.timestamp
      ? `<h2>Timestamp</h2><pre>${escHtml(
          data.tsResult || "Timestamp created successfully.",
        )}</pre>`
      : "") +
    fpSection +
    (data.didSig?.did
  ...
  const spineItems = manifestItems.filter(
    (m) => m.mt === "application/xhtml+xml",
  );
