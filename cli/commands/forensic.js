...
function elaDiff(original, recompressed) {
  ...
  for (let i = 0; i < original.data.length; i += 4) {
    const d = (Math.abs(original.data[i] - recompressed.data[i]) +
      Math.abs(original.data[i + 1] - recompressed.data[i + 1]) +
      Math.abs(original.data[i + 2] - recompressed.data[i + 2])) / 3;
    scores.push(d);
    if (d > max) max = d;
  }
...
async function analyzeForensicFile(absPath) {
  ...
  if (loaded.imageData.width > FORENSIC_MAX_DIMENSION || loaded.imageData.height > FORENSIC_MAX_DIMENSION) {
    throw new Error('Image dimensions (' + loaded.imageData.width + 'x' + loaded.imageData.height + ') exceed maximum allowed (' + FORENSIC_MAX_DIMENSION + 'px)');
  }
  ...
  const combined = core.combineFindings({ ela, noise, copy_move: copyMove, metadata });
  const signals = core.buildSummary({ ela, noise, copy_move: copyMove, metadata });

  return {
    file: { name: info.name, size: info.size, size_human: fmtSize(info.size), type: info.type },
