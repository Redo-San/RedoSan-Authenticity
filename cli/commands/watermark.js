...
if (typeof globalThis.sha256Hex === 'undefined') {
  globalThis.sha256Hex = async function(data) {
...
const ADVANCED_ALGOS = [
  'enhanced_lsb', 'adaptive_lsb', 'multi_channel_lsb',
  'dwt', 'dft', 'hybrid_dct_dwt',
...
function makePayload(secretData, key) {
  ...
  rawData.set(magic); rawData.set(secretData, 2);
...
async function doEmbed(canvas, ctx, imgData, opts, algoName, algoNum, isAdvanced, password) {
  ...
  if (!outputFile) { console.error('Error: --output (-o) is required for embed mode'); process.exit(1); }
  ...
  if (isAdvanced) {
    ...
    if (!embedFn) { console.error(`Algorithm ${algoName} not found in WatermarkCore`); process.exit(1); }
    ...
    if (payload.bits.length > maxPixels) { console.error('Image too small'); process.exit(1); }
    ...
    // Multi-bit (2-bit LSB)
    const payload = makePayload(secretData || new Uint8Array(0), key);
    if (payload.bits.length > maxPixels * 2 / 3) { console.error('Image too small'); process.exit(1); }
    ...
    if (!secretData) { console.error('Secret required for fragile algorithm'); process.exit(1); }
    if (512 > maxPixels) { console.error('Image too small (need >= 171 pixels)'); process.exit(1); }
    ...
    if (algoNum === 5) {
      ...
      const payload = makePayload(secretData || new Uint8Array(0), key);
      if (algoNum === 4) {
        if (payload.bits.length * 3 > cap) { console.error('Secret too large for redundant embedding'); process.exit(1); }
        globalThis.embedInDCT(ycbcr.Y, w, h, payload.bits + payload.bits + payload.bits, 30);
      } else if (algoNum === 7) {
        if (payload.bits.length > cap) { console.error('Secret too large'); process.exit(1); }
...
async function doExtract(canvas, ctx, imgData, opts, algoName, algoNum, isAdvanced, password) {
  ...
  if (isAdvanced) {
    ...
    if (spatialAdvanced.includes(algoName)) {
      ...
      if (data) { writeExtracted(data, outputFile, algoName); }
      else { console.log('No watermark found. Try a different algorithm or password.'); }
      ...
      const extractMethods = {
        ...
        stardustmark: 'extractStardustMark', invisimark: 'extractInvisiMark',
  ...
  if (algoNum === 1 || algoNum === 3 || algoNum === 6) {
    ...
    let bits = globalThis[fn](...args);
    if (typeof bits === 'string') { /* already string */ }
    const data = extractPayload(bits, key);
    if (data) { writeExtracted(data, outputFile, algoName); }
    else { console.log('No watermark found. Try a different algorithm or password.'); }
    ...
    if (b.length < sig.length * 8) { console.log('No watermark detected'); return; }
    ...
    if (ratio > 0.85) { console.log(`Presence confirmed (${Math.round(ratio*100)}% match)`); }
    else { console.log(`No watermark (${Math.round(ratio*100)}% match)`); }
    ...
    } else { console.log('No fragile watermark found.'); }

  } else if ([2,4,7,9].includes(algoNum)) {
    const ycbcr = globalThis.rgbToYcbcr(imgData);
    let b = globalThis.extractFromDCT(ycbcr.Y, w, h, 32);
    if (b.length < 32) { console.log('No watermark found'); return; }
    const dlen = parseInt(b.substr(0, 32), 2);
    if (dlen <= 0 || dlen > 100000) { console.log('No watermark found'); return; }

    if (algoNum === 4) {
      const totalBits = 32 + dlen * 8 * 3;
      b = globalThis.extractFromDCT(ycbcr.Y, w, h, totalBits);
      if (b.length < totalBits) { console.log('No watermark found'); return; }
    } else if ([2,7].includes(algoNum)) {
      b = globalThis.extractFromDCT(ycbcr.Y, w, h, 32 + dlen * 8);
      if (b.length < 32 + dlen * 8) { console.log('No watermark found'); return; }
    } else if (algoNum === 9) {
    ...
    if (data) { writeExtracted(data, outputFile, algoName); }
    else { console.log('Wrong password or no watermark.'); }
  }
}
