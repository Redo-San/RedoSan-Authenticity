...
var NANOID_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
...
function uuidv4() {
  ...
  var i = 0;
  return hex(bytes[i++], 2) + hex(bytes[i++], 2) + hex(bytes[i++], 2) + hex(bytes[i++], 2) + '-' +
    hex(bytes[i++], 2) + hex(bytes[i++], 2) + '-' +
    hex(bytes[i++], 2) + hex(bytes[i++], 2) + '-' +
    hex(bytes[i++], 2) + hex(bytes[i++], 2) + '-' +
    hex(bytes[i++], 2) + hex(bytes[i++], 2) + hex(bytes[i++], 2) + hex(bytes[i++], 2) + hex(bytes[i++], 2) + hex(bytes[i++], 2);
}
...
function uuidv7Bulk(n) { var r = []; for (var i = 0; i < n; i++) r.push(uuidv7()); return r; }
function uuidv4Bulk(n) { var r = []; for (var i = 0; i < n; i++) r.push(uuidv4()); return r; }

function ulid() {
  var ts = Date.now();
  var t = '';
  var n = ts;
  for (var i = 0; i < 10; i++) { t = CROCKFORD_B32[n % 32] + t; n = Math.floor(n / 32); }
  var r = '';
...
function ulidBulk(n) { var r = []; for (var i = 0; i < n; i++) r.push(ulid()); return r; }
...
function nanoidBulk(n, len) { var r = []; for (var i = 0; i < n; i++) r.push(nanoid(len)); return r; }
...
function handleIdForgeGenerate() {
  ...
  btn.disabled = true;
  btn.textContent = i18n.data && i18n.data['id_forge.generating_btn'] || 'Generating...';

  setTimeout(async function () {
    try {
      var ids;
      switch (type) {
        case 'uuidv4': ids = count === 1 ? [uuidv4()] : uuidv4Bulk(count); break;
        case 'uuidv7': ids = count === 1 ? [uuidv7()] : uuidv7Bulk(count); break;
        case 'ulid': ids = count === 1 ? [ulid()] : ulidBulk(count); break;
        case 'nanoid': ids = count === 1 ? [nanoid()] : nanoidBulk(count); break;
        case 'swhid':
          ids = [];
      ...
      window._ifResult = { ids: ids, type: type, count: count, timestamp: new Date().toISOString() };
    ...
    btn.disabled = false;
    btn.textContent = i18n.data && i18n.data['id_forge.generate_btn'] || 'Generate';
  }, 50);
}
...
function idForgeCopy(el) {
  ...
  el.textContent = i18n.data && i18n.data['id_forge.copied_btn'] || '✓ Copied!';
...
function idForgeDownload(format) {
  ...
  if (format === 'pdf') {
    ...
    var y = 30;
    doc.text('Generated: ' + r.timestamp, 14, y); y += 6;
    doc.text('Type: ' + r.type, 14, y); y += 6;
    doc.text('Count: ' + r.count, 14, y); y += 10;
    doc.setFontSize(8);
  ...
  switch (format) {
    ...
    case 'csv':
      ...
      for (var s = 0; s < r.ids.length; s++) content += r.type + ',' + r.count + ',' + r.timestamp + ',' + r.ids[s] + '\n';
      ext = 'csv'; mime = 'text/csv';
      break;
    case 'txt': content = 'ID Forge — ' + r.type + '\nGenerated: ' + r.timestamp + '\nCount: ' + r.count + '\n\n' + r.ids.join('\n'); ext = 'txt'; mime = 'text/plain'; break;
    case 'xml':
      content = '<?xml version="1.0" encoding="UTF-8"?>\n<id-forge>\n  <metadata>\n    <type>' + escXml(r.type) + '</type>\n    <count>' + r.count + '</count>\n    <timestamp>' + escXml(r.timestamp) + '</timestamp>\n  </metadata>\n  <ids>\n';
      for (var t = 0; t < r.ids.length; t++) content += '    <id>' + escXml(r.ids[t]) + '</id>\n';
      content += '  </ids>\n</id-forge>';
...
function idForgeShowInfo() {
  ...
  var icon = { uuidv4: '🎲', uuidv7: '⏱️', ulid: '🔤', nanoid: '🔗', swhid: '📦' };
  var text = (i18n.data && i18n.data['id_forge.info.' + type]) || '';
  info.style.display = text ? 'block' : 'none';
  info.innerHTML = '<span style="font-size:1.1rem;margin-right:6px">' + (icon[type] || '') + '</span> ' + text;
}
