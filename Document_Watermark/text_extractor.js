...
function readDocx(raw) {
  return new Promise(function(resolve, reject) {
    ...
    JSZip.loadAsync(raw).then(function(zip) {
      var entry = zip.file('word/document.xml');
      if (!entry) {
        reject(new Error('Not a valid DOCX file: word/document.xml not found'));
        return;
      }
...
function extractDocxText(xml) {
  ...
  while (i < xml.length) {
    ...
    if (xml.substr(i, 10) === '<w:p>') inPara = true;
    if (xml.substr(i, 11) === '</w:p>' && inPara) { text += '\n'; inPara = false; }
...
async function inflateRaw(data) {
  ...
  if (typeof DecompressionStream === 'undefined') {
    throw new Error('PDF compression not supported in this browser. Try a plain text file instead.');
  }
  // Try zlib format first (per PDF spec)
  try { return await inflateStream(data, 'deflate'); } catch (e1) { console.warn('docw: deflate failed', e1); }
  // Try raw deflate (some non-compliant PDF generators)
  try { return await inflateStream(data, 'deflate-raw'); } catch (e2) { console.warn('docw: deflate-raw failed', e2); }
...
async function readPdf(raw) {
  ...
  for (var objId in objMap) {
    ...
    var dec2;
    try { dec2 = await inflateRaw(stringToBytes(raw2)); } catch(e) { continue; }
  ...
  for (var p = 0; p < pages.length; p++) {
    // Yield every 5 pages so the UI stays responsive during large PDFs
    ...
    function decodePdfString(s) {
      ...
      // Detect UTF-16BE by pattern: null byte + ASCII char repeating
      var asianCount = 0;
    ...
    function unescapePdfStr(s) {
      return s.replace(/\\([nrt])/g, function(m, ch) { return ' '; }).replace(/\\(.)/g, '$1');
    }
...
async function docwExtractText(file, callback) {
  ...
  if (ext === 'docx') {
    reader.onload = function(e) {
      DOCX_EXTRACTOR.readDocx(e.target.result).then(function(text) {
    ...
    reader.onload = function(e) {
      DOCX_EXTRACTOR.readPdf(new Uint8Array(e.target.result)).then(function(text) {
        if (text) {
          callback(null, text, 'pdf');
        } else {
          callback('Could not extract text from PDF. The PDF may be image-based or encrypted.');
        }
      }).catch(function(err) {
