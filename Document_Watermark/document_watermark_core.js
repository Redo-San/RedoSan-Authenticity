...
function _utf8Encode(str) {
    ...
    for (var i = 0; i < str.length; i++) {
        ...
        else if (cp < 0xD800 || cp >= 0xE000) bytes.push(0xE0 | (cp >> 12), 0x80 | ((cp >> 6) & 0x3F), 0x80 | (cp & 0x3F));
...
function _utf8Decode(bytes) {
    ...
    for (var i = 0; i < bytes.length; i++) {
        ...
        else if (b >= 0xC0 && b < 0xE0 && i + 1 < bytes.length) str += String.fromCharCode(((b & 0x1F) << 6) | (bytes[++i] & 0x3F));
        else if (b >= 0xE0 && b < 0xF0 && i + 2 < bytes.length) str += String.fromCharCode(((b & 0x0F) << 12) | ((bytes[++i] & 0x3F) << 6) | (bytes[++i] & 0x3F));
...
async function _msgToBits(message, password) {
    ...
    var compressed;
    try { compressed = await _deflate(bytes); } catch (e) { compressed = null; }
    var payload;
...
async function _bitsToMsg(bits, password) {
    ...
    if (bytes.length > 0 && bytes[0] === 0x02) {
        ...
        try {
            ...
            for (var ti = decoded.length - 1; ti >= 0; ti--) { if (decoded[ti] !== 0) { end = ti + 1; break; } }
...
var DOCW_ZWC = {
    ...
    embed: async function (text, message, password) {
        ...
        if (perChar > this.MAX_ZWCS_PER_CHAR) {
            throw new Error('Cover text too short. Need ~' + (needed / this.MAX_ZWCS_PER_CHAR) + ' chars, have ' + text.length);
        }
        var result = '';
        var bitIdx = 0;
    ...
    embed: async function (text, message, password) {
        ...
        for (var e2 = 0; e2 < eligible.length; e2++) {
            ...
            if (multi) {
                ...
                if (bitIdx < bits.length && bits[bitIdx] === '1') result[idx] = this.MAP[ch2];
                bitIdx += 1;
            }
        }
        return result.join('');
...
var DOCW_WHITESPACE = {
    ...
    embed: async function (text, message, password) {
        ...
        var encoded = [];
        for (var i = 0; i + (this.BITS_PER_SPACE - 1) < bits.length; i += this.BITS_PER_SPACE) {
...
async function docwAutoDetect(text, password) {
    ...
    for (var id in DOCW_ALGOS) {
        try {
            ...
            if (result) return { algo: id, name: DOCW_ALGOS[id].name, message: result };
        } catch (e) {
...
function docwEstimateCapacity(text, algoId) {
    ...
    if (!algo) return 0;
    var impl = algo.impl;
    if (impl === DOCW_ZWC) return Math.floor(text.length * impl.MAX_ZWCS_PER_CHAR * impl.BITS_PER_ZWC / 8);
