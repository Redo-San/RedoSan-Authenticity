...
(async () => {
  ...
  for (const blocked of scripts) {
    await pg.route("**/*", (route) => {
      ...
      if (blocked === "removal_tools.js" && u.includes("removal_tools"))
        return route.abort();
      if (blocked !== "removal_tools.js" && u.includes(blocked))
        return route.abort();
    ...
    const r = await pg.evaluate(() => ({
      cls: window.__cls,
      footerH: document.getElementById("mainFooter")
        ? Math.round(document.getElementById("mainFooter").offsetHeight)
        : null,
      footMut: window.__mut
        .filter(
          (m) =>
            (m.target === "mainFooter" || m.target === "FOOTER") &&
            (m.type === "attributes" || m.removed > 0 || m.added > 0),
        )
        .slice(0, 8),
    }));
    console.log(
      `--- blocking ${blocked}: cls=${(r.cls || 0).toFixed(3)} footerH=${
        r.footerH
      }`,
    );
