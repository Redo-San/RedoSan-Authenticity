...
(async () => {
  ...
  for (let run = 1; run <= 5; run++) {
    ...
    await pg.addInitScript(() => {
      ...
      const tick = () => {
        ...
        if (t > 150 && t < 1400 && !done) {
          window.__probe.push({ t, initDZ: typeof window.initDropZones, setStatus: typeof window.setStatus, dz: document.querySelectorAll('.file-drop-zone').length });
    ...
    const changes = [];
    let last = null;
    for (const p of r) { const sig = p.initDZ + '|' + p.setStatus + '|' + p.dz; if (sig !== last) { changes.push(p); last = sig; } }
    console.log('=== RUN ' + run + ' errs=' + (errs.length ? errs.join(' ; ') : 'none'));
    for (const p of changes) console.log('   @' + p.t + ' initDZ=' + p.initDZ + ' setStatus=' + p.setStatus + ' dz=' + p.dz);
    await ctx.close();
  }
  await browser.close();
})();
