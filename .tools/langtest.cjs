...
(async () => {
  ...
  for (let run = 1; run <= 4; run++) {
    ...
    await pg.addInitScript((m) => {
      try {
        localStorage.setItem("redosan_lang", m);
      } catch (e) {}
      window.__clsRaw = [];
      window.__texts = [];
      try {
      ...
      const tick = () => {
        ...
        if (t % 50 < 15 && t > 200 && t < 1200) {
          ...
          window.__texts.push({
            t,
            s: (el ? el.textContent : "").slice(0, 30),
            dir: document.documentElement.dir,
            ots: ots ? Math.round(ots.getBoundingClientRect().height) : -1,
          });
        }
    ...
    for (const t of r.texts) {
      const k = t.s + "|" + t.ots;
      if (!seen.includes(k) || seen.length < 12) {
        seen.push(k);
        console.log("  @" + t.t + " dir=" + t.dir + " ots=" + t.ots + ' text="' + t.s + '"');
      }
    }
    await ctx.close();
  }
