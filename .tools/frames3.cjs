      ...
      if (t >= 250 && t <= 1300) {
        ...
        if (ots) {
          ...
          ots.querySelectorAll(':scope > *').forEach((el) => {
            ...
            kids.push({ c: (el.id ? '#' + el.id : '') + (el.className && typeof el.className === 'string' ? '.' + el.className.trim().split(/\s+/).join('.') : '') + '<' + el.tagName + '>', t: Math.round(r.top), h: Math.round(r.height), d: getComputedStyle(el).display });
          });
          ...
          if (sig !== last) { window.__frames.push({ t, kids, hasDz, hasRaw }); last = sig; }
        }
      }
