...
(async () => {
  ...
  for (let run = 1; run <= 3; run++) {
    ...
    await pg.addInitScript(() => {
      ...
      const tick = () => {
        ...
        if (t > 150 && t < 1600) {
          ...
          if (sig !== last) { window.__probe.push({ t, sig, btn: (btn || {}).textContent || '' }); last = sig; }
        }
    ...
    const r = await pg.evaluate(() => ({ probe: window.__probe, res: performance.getEntriesByType('resource').map(e => e.name.split('/').pop() + '@' + Math.round(e.responseEnd)).filter(s => s.includes('lang') || s.includes('i18n')) }));
    console.log('=== RUN ' + run + ' resources: ' + JSON.stringify(r.res));
    for (const p of r.probe) console.log('  @' + p.t + ' sec|btn|dz0=' + p.sig + ' btnTxt="' + p.btn.slice(0, 20) + '"');
    await ctx.close();
  }
  await browser.close();
})();
