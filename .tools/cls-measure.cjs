...
(async () => {
  ...
  for (const page of ["watermark", "certificate", "timestamp"]) {
    ...
    const r = await pg.evaluate(() => {
      ...
      const pw =
        document.getElementById("wm-password-group") ||
        document.getElementById("awm-password-group") ||
        document.getElementById("cert-password") ||
        null;
      return {
        cls: window.__cls,
        ...
        dz,
        inputs,
        pwVisible: pw ? getComputedStyle(pw).display : "n/a",
      };
    });
    console.log(`=== ${page} ===`);
