...
(async () => {
  ...
  for (let run = 1; run <= 3; run++) {
          ...
          const obs = new MutationObserver((muts) => {
            ...
            for (const m of muts) {
              if (m.target === foot || (m.target && m.target.parentElement === foot)) {
                window.__mut.push({ t: Math.round(performance.now()), type: m.type, attr: m.attributeName || null, value: m.attributeName === 'style' ? (m.target.getAttribute('style') || '') : null, removed: m.removedNodes.length, added: m.addedNodes.length });
              }
            }
          });
          obs.observe(document.documentElement, { subtree: true, childList: true, attributes: true, attributeFilter: ['style', 'class'] });
        } catch (e) { window.__mutErr = String(e); }
        try {
    ...
    const r = await pg.evaluate(() => ({
      cls: window.__cls, clsRaw: window.__clsRaw, mut: (window.__mut || []).slice(0, 10), mutErr: window.__mutErr,
      footerDisplay: getComputedStyle(document.getElementById('mainFooter')).display,
    }));
    console.log(`=== RUN ${run}: cls=${r.cls.toFixed(4)} footerDisplay=${r.footerDisplay} mutErr=${r.mutErr}`);
