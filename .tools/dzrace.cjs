...
(async () => {
  ...
  for (let run = 1; run <= 6; run++) {
    ...
    await pg.addInitScript(() => {
      ...
      const tick = () => {
        ...
        if (t > 150 && t < 1400) {
          ...
          if (n !== last) { window.__dzTimeline.push({ t, n }); last = n; }
        }
    ...
    const r = await pg.evaluate(() => ({ cls: window.__clsRaw, tl: window.__dzTimeline }));
    const total = r.cls.reduce((a, b) => a + b.v, 0);
    console.log('=== RUN ' + run + ' cls=' + total.toFixed(4) + ' dzTimeline=' + r.tl.map((x) => x.t + ':' + x.n).join(' '));
    await ctx.close();
  }
  await browser.close();
})();
