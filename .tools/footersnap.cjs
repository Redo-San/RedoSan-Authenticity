...
(async () => {
  ...
  for (let run = 1; run <= 4; run++) {
    ...
    await pg.addInitScript(() => { localStorage.setItem('redosan_lang', 'ar'); });
    ...
    const snap = () => pg.evaluate(() => {
      ...
      if (fb) {
        ...
        out.kids = Array.from(fb.children).map(c => c.tagName + '.' + (c.className || '').split(' ')[0] + '=' + Math.round(c.getBoundingClientRect().height)).join(' ');
