...
(async () => {
  ...
  const r = await pg.evaluate(() => {
    ...
    const groups = {};
    b.forEach((x) => { (groups[x.k] = groups[x.k] || []).push(x); });
    const agroups = {};
    a.forEach((x) => { (agroups[x.k] = agroups[x.k] || []).push(x); });
    const diffs = [];
    for (const k of Object.keys(groups)) {
      const bg = groups[k], ag = agroups[k] || [];
      for (let i = 0; i < bg.length; i++) {
        if (i >= ag.length) { diffs.push({ k, gone: true }); continue; }
        const x = bg[i], y = ag[i];
        if (Math.abs(y.h - x.h) > 4 || Math.abs(y.t - x.t) > 4) diffs.push({ k, t: x.t + '->' + y.t, h: x.h + '->' + y.h });
      }
    }
    return diffs;
  });
