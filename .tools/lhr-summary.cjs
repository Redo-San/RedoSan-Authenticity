...
for (const [url, runs] of Object.entries(pages)) {
  const best = {
    perf: -1, a11y: -1, bp: null, seo: -1,
    tbt: Infinity, lcp: Infinity, cls: Infinity,
  };
  const perRun = [];
  for (const { file, l } of runs) {
    ...
    best.perf = Math.max(best.perf, m(row.perf, (x) => x));
    best.a11y = Math.max(best.a11y, m(row.a11y, (x) => x));
    if (row.bp !== null && row.bp !== undefined) best.bp = best.bp === null ? row.bp : Math.max(best.bp, row.bp);
    best.seo = Math.max(best.seo, m(row.seo, (x) => x));
    best.tbt = Math.min(best.tbt, m(row.tbt, (x) => x));
    best.lcp = Math.min(best.lcp, m(row.lcp, (x) => x));
    best.cls = Math.min(best.cls, m(row.cls, (x) => x));
  }
...
console.log('|------|------|------|----|-----|---------|---------|-----|');
for (const r of rows) {
  console.log('| ' + r.label + ' | ' + fmtScore(r.best.perf) + ' | ' + fmtScore(r.best.a11y) + ' | ' + fmtScore(r.best.bp) + ' | ' + fmtScore(r.best.seo) + ' | ' + (r.best.tbt === Infinity ? '-' : r.best.tbt.toFixed(0)) + ' | ' + (r.best.lcp === Infinity ? '-' : r.best.lcp.toFixed(0)) + ' | ' + (r.best.cls === Infinity ? '-' : r.best.cls.toFixed(3)) + ' |');
}
