  ...
  await pg.addInitScript(() => {
    ...
    const tick = () => {
      ...
      if (t > 380 && t < 1000) {
        ...
        const sig = bt + '|' + (!!(window.__I18N_DATA && window.__I18N_DATA.ar)) + '|' + (typeof window.i18n !== 'undefined' && !!i18n.data && !!i18n.data['ts.ts_create_btn']);
        if (sig !== last) { window.__probe.push({ t, sig }); last = sig; }
      }
