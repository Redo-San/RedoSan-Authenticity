  ...
  for (let run = 1; run <= 3; run++) {
    ...
    await pg.addInitScript(() => {
      ...
      const obs = new MutationObserver((muts) => {
        for (const m of muts) {
          if (
            m.target === document.getElementById("mainFooter") ||
            (m.target &&
              m.target.parentElement === document.getElementById("mainFooter"))
          ) {
            window.__mut.push({
        ...
        new PerformanceObserver((l) => {
          for (const e of l.getEntries())
            if (!e.hadRecentInput) {
              ...
              window.__clsRaw.push({ v: e.value, t: Math.round(e.startTime) });
            }
    ...
    console.log(
      `=== RUN ${run}: cls=${r.cls.toFixed(3)} footerDisplay=${
        r.footerDisplay
      }`,
    );
