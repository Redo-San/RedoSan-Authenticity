...
(async () => {
    ...
    const snap = () => {
      ...
      document.querySelectorAll('body *').forEach((el) => {
        ...
        if (r.width > 0 && r.height > 0 && r.top < 2000) {
          out.push({ id: el.id, cls: typeof el.className === 'string' ? el.className : '', tag: el.tagName, t: Math.round(r.top), h: Math.round(r.height), d: getComputedStyle(el).display });
        }
      });
      return out;
    };
  ...
  const r = await pg.evaluate(() => {
    ...
    const map = {};
    a.forEach((x) => { map[(x.id ? '#' + x.id : '') + '<' + x.tag + '>'] = x; });
    const diffs = [];
    b.forEach((x) => {
      const k = (x.id ? '#' + x.id : '') + '<' + x.tag + '>';
      const y = map[k];
      if (y) {
        const dh = y.h - x.h, dt = y.t - x.t;
