...
(async () => {
  ...
  for (let run = 1; run <= runs; run++) {
              ...
              window.__clsRaw.push({
                ...
                srcs: (e.sources || []).map((s) => {
                  ...
                  return (
                    ...
                    (n.tagName ? "<" + n.tagName + ">" : "") +
                    "[" +
                    Math.round(s.previousRect.top) +
                    "->" +
                    Math.round(s.currentRect.top) +
                    ",h" +
    ...
    console.log(
      `=== RUN ${run}: cls=${total.toFixed(4)} lang=${lang.lang} dir=${
        lang.dir
      } activePages=${lang.active}`,
    );
    for (const c of r.clsRaw)
      console.log("  ", c.v, "@", c.t, c.srcs.slice(0, 4).join(" | "));
    if (run === 1)
      console.log(
        "  res:",
        r.res
          .filter((x) =>
            ["ar.json", "rtl.css", "i18n.js", "navigation.js"].includes(x.n),
          )
          .map((x) => `${x.n}@${x.t}+${x.d}`)
          .join(" "),
      );
