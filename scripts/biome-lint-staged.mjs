...
const biomeConfig = JSON.parse(readFileSync(configPath, "utf8"));
...
const allowed = visible.filter((p) => {
  ...
  return includes.some((inc) => !inc.startsWith("!") && globMatches(p, inc));
});
if (allowed.length === 0) process.exit(0);

const cmd = `npx @biomejs/biome check --write --config-path "${configPath.replace(
  /\\/g,
  "/",
