...
function FaceEngine(options) {
    ...
    this._modelBasePath = options && options.modelBasePath ? options.modelBasePath : 'https://cdn.jsdelivr.net/npm/@vladmandic/human@3.3.6/models/';
    this._tinyDetector = options && options.tinyDetector ? options.tinyDetector : false;
    this._loaded = false;
}
...
FaceEngine.prototype.loadModels = async function () {
    ...
    humanModule = typeof window !== 'undefined' && window.Human ? window.Human : null;
    HumanCtor = this._human ? this._human.constructor : (humanModule ? (humanModule.Human || humanModule.default || humanModule) : null);
    if (!HumanCtor) throw new Error('@vladmandic/human is not loaded. Include the library via script tag or pass a human instance.');
    if (!this._human) {
    ...
    async function loadWithTimeout(human, cfg, ms) {
      return Promise.race([
        ...
        new Promise(function(_, reject) {
          setTimeout(function() { reject(new Error('Model loading timed out')); }, ms);
        }),
      ]);
    }
    loaded = false;
...
FaceEngine.prototype.detectFaces = async function (input) {
    ...
    if (!this._loaded) throw new Error('Models not loaded. Call loadModels() first.');
    tfLib = this._human && this._human.tf;
    canRetry = !!tfLib && typeof tfLib.setBackend === 'function' &&
        this._backend === 'webgl' && !this._webglUnhealthy;
    try {
 ...
 */
FaceEngine.matchInRegistry = function (query, registry, threshold, embeddingVersion) {
