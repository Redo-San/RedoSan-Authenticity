...
(function () {
    ...
    function sha256Hex(input) {
        var bytes = [], i, c, bitLen, hi, lo, H, off, t, idx, w, s0, s1;
    ...
    function generate(descriptor, pin, opts) {
        ...
        if (typeof pin !== 'string' || pin.length === 0) throw new Error('A PIN is required to generate a BioHash identifier.');
        if (!descriptor || typeof descriptor.length !== 'number' || descriptor.length === 0) throw new Error('A face descriptor is required.');
        if (dim > descriptor.length) throw new Error('BioHash dim (' + dim + ') exceeds descriptor length (' + descriptor.length + ').');
    ...
    function match(query, registry, threshold) {
        ...
        if (!registry || registry.length === 0) return { match: null, similarity: 0, distance: -1 };
        best = null;
        bestSim = -1;
        bestDist = -1;
        ...
        if (bestSim < threshold) return { match: null, similarity: bestSim, distance: bestDist };
    ...
    if (typeof module !== 'undefined' && module.exports) module.exports = FaceBioHash;
    /* c8 ignore stop */
})();
