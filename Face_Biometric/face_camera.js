...
FaceCamera.isSupported = function () {
    ...
    nav = navigator;
    md = nav.mediaDevices;
    return !!md && typeof md.getUserMedia === "function" && nav.isSecureContext !== false;
};
...
FaceCamera.prototype.startCamera = async function (videoEl, opts) {
    ...
    if (!FaceCamera.isSupported()) {
        throw new Error("Camera not available: insecure context or no mediaDevices support.");
    }
...
FaceCamera.prototype.scoreFrame = async function (canvas, engine) {
    ...
    if (!engine || !engine.detectFaces) return { canvas: canvas, result: null, score: 0 };
    ...
    if (face.box.width > 0 && face.box.height > 0) {
        score += Math.min(50, (face.box.width * face.box.height) / (canvas.width * canvas.height) * 1000);
    }
