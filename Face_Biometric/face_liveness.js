...
FaceLiveness.ear = function (mesh, eyeIndices) {
    ...
    if (p1 === undefined || p2 === undefined || p3 === undefined || p4 === undefined || p5 === undefined || p6 === undefined) return 0;
    d1 = Math.hypot(p2 - p6, mesh[idx(eyeIndices[1]) + 1] - mesh[idx(eyeIndices[5]) + 1]);
    d2 = Math.hypot(p3 - p5, mesh[idx(eyeIndices[2]) + 1] - mesh[idx(eyeIndices[4]) + 1]);
    d3 = Math.hypot(p1 - p4, mesh[idx(eyeIndices[0]) + 1] - mesh[idx(eyeIndices[3]) + 1]);
    if (d3 === 0) return 0;
    return (d1 + d2) / (2 * d3);
};
...
FaceLiveness.blinkScore = function (frames) {
    ...
    if (!frames || frames.length === 0) return { count: 0, earSeries: [], closedRuns: 0 };
    earSeries = [];
    closed = [];
...
FaceLiveness.motionScore = function (frames) {
    ...
    mean /= deltas.length;
    variance = 0;
    for (i = 0; i < deltas.length; i++) variance += (deltas[i] - mean) * (deltas[i] - mean);
    n = deltas.length;
    return variance / n;
};
...
FaceLiveness.sharpnessScore = function (canvas) {
    ...
    mean /= lap.length;
    variance = 0;
    for (i = 0; i < lap.length; i++) variance += (lap[i] - mean) * (lap[i] - mean);
...
FaceLiveness.qualityScore = function (result, frameW, frameH) {
    ...
    reasons = [];
    if (!result || !result.box) return { ok: false, score: 0, reasons: ["no_face"] };
    box = result.box;
    faceW = box.width;
    faceH = box.height;
    ...
    conf = result.score || 0;
    if (conf < FACE_LIVENESS_CONFIG.qualityMinConfidence) reasons.push("low_confidence");
    if (ratio > FACE_LIVENESS_CONFIG.qualityMaxFaceRatio) reasons.push("too_close");
    else if (ratio < FACE_LIVENESS_CONFIG.qualityMinFaceRatio) reasons.push("too_far");
    if (result.mesh) {
        noseX = result.mesh[FACE_MESH_INDICES.noseTip * 3];
        centerX = box.x + faceW / 2;
        if (noseX !== undefined) {
            offset = Math.abs(noseX - centerX) / faceW;
            if (offset > FACE_LIVENESS_CONFIG.frontalNoseOffsetRatio) reasons.push("not_frontal");
        }
    }
    ok = reasons.length === 0;
    return {
...
FaceLiveness.analyzePassive = function (frames) {
    ...
    if (!frames || frames.length === 0) {
        return { live: false, score: 0, blinkCount: 0, motion: 0, sharpness: 0, quality: { ok: false, reasons: ["no_frames"] }, reasons: ["no_frames"] };
    }
    ...
    for (i = 0; i < frames.length; i++) {
        ...
        if (frame && frame.result && !quality.ok) {
            q = FaceLiveness.qualityScore(frame.result, frame.canvas ? frame.canvas.width : 640, frame.canvas ? frame.canvas.height : 480);
            if (i === 0) quality = q;
    ...
    if (sharpness > 0 && sharpness < FACE_LIVENESS_CONFIG.sharpnessMin) reasons.push("blurred");
...
FaceLiveness.ChallengeEngine = function (opts) {
    var o = opts || {};
    this._challenges = o.challenges && o.challenges.length ? o.challenges.slice() : null;
    this._timeoutMs = o.timeoutMs || FACE_LIVENESS_CONFIG.activeChallengeTimeoutMs;
...
FaceLiveness.ChallengeEngine.VALIDATORS = {
    ...
    smile: function (mesh) {
        ...
        if (c1y === undefined || c2y === undefined || midY === undefined || lowY === undefined) return false;
...
FaceLiveness.ChallengeEngine.prototype.start = function () {
    if (!this._challenges) {
        this._challenges = ["blink", "smile", "turn-left", "turn-right", "look-up", "look-down"];
        while (this._challenges.length > FACE_LIVENESS_CONFIG.activeMinChallenges) {
            this._challenges.splice(Math.floor(Math.random() * this._challenges.length), 1);
        }
    }
    this._index = 0;
...
FaceLiveness.ChallengeEngine.prototype.validate = function (mesh, now) {
    ...
    if (this._done) return { current: null, passed: false, done: true, live: this._passed.length > 0, next: null };
    ...
    if (timedOut) {
        ...
        this._attempts++;
        if (this._attempts >= this._maxAttempts || this._index >= this._challenges.length - 1) {
            this._done = true;
            return { current: null, passed: false, done: true, live: false, next: null };
        }
        this._index++;
        this._currentDeadline = now + this._timeoutMs;
        return { current: this._challenges[this._index], passed: false, done: false, live: false, next: this._challenges[this._index] };
    }
    validator = FaceLiveness.ChallengeEngine.VALIDATORS[type];
    if (!validator || !validator(mesh)) {
        return { current: type, passed: false, done: false, live: false, next: null };
    }
    ...
    this._currentDeadline = now + this._timeoutMs;
    return { current: type, passed: true, done: false, live: false, next: this._challenges[this._index] };
};
...
FaceLiveness.ChallengeEngine.prototype.summary = function () {
    return {
        ...
        passed: this._passed.map(function (p) { return p.type; }),
        failed: this._failed.map(function (f) { return f.type; }),
        done: this._done,
    };
};
...
FaceLiveness.prototype.verifyLiveness = async function (camera, engine, opts) {
    ...
    if (!camera || !camera.captureFrame) {
        return { live: false, score: 0, mode: mode, reasons: ["no_camera"], durationMs: 0, timestamp: new Date().toISOString() };
    }
    frames = [];
    if (mode === "passive" || mode === "both") {
        for (i = 0; i < (opts && opts.frames ? opts.frames : FACE_LIVENESS_CONFIG.captureFrames); i++) {
    ...
    evidence = {
        ...
        quality: passive ? passive.quality : { ok: false, reasons: ["not_analyzed"] },
    ...
    if (mode === "active" || mode === "both") {
        if (!passive || passive.live) {
            challengeEngine = new FaceLiveness.ChallengeEngine({
                ...
                timeoutMs: opts && opts.timeoutMs ? opts.timeoutMs : FACE_LIVENESS_CONFIG.activeChallengeTimeoutMs,
            });
            ...
            currentType = type;
            if (notifyChallenge) notifyChallenge({ type: type, index: 0, total: total, done: false });
            while (type !== null && !challengeEngine._done) {
                if (Date.now() > activeDeadline) {
                    challengeEngine._done = true;
                    if (notifyChallenge) notifyChallenge({ type: null, index: challengeEngine._index, total: total, done: true });
                    break;
                }
                canvas = camera.captureFrame();
                detection = null;
                ...
                if (detection && detection.length > 0 && detection[0].mesh) {
                    ...
                    if (notifyChallenge && (result.done || (result.next && result.next !== currentType))) {
            ...
            evidence.live = challengeEngine._done && challengeEngine.summary().passed.length > 0;
            evidence.score = challengeEngine.summary().passed.length / Math.max(1, challengeEngine.summary().total);
