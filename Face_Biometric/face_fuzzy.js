...
(function () {
  ...
  var sha256Hex =
    typeof FaceBioHash !== "undefined" && FaceBioHash._sha256Hex
  ...
  function quantize(descriptor) {
    ...
    sorted = Array.prototype.slice.call(descriptor).sort(function (a, b) {
      return a - b;
    });
    median =
      sorted.length % 2 === 1
  ...
  function encode(bits, opts) {
    ...
    if (!bits || bits.length === 0)
      throw new Error("Biometric bits are required.");
    totalBits = bits.length * 8;
    secretBitLen = Math.floor(totalBits / rep);
    if (secretBitLen < 8)
      throw new Error(
        "Not enough biometric bits for repetition factor " +
          rep +
          " (need at least 8*" +
          rep +
          ").",
    ...
    if (opts.secret) {
      ...
      for (i = 0; i < secretBytes.length; i++)
        secretBytes[i] = parseInt(hex.substring(i * 2, i * 2 + 2), 16);
    } else {
    ...
    for (k = 0; k < secretBitLen; k++)
      setBit(hashedBytes, k, getBit(secretPacked, k));
    codeword = repeatCode(secretPacked, secretBitLen, rep);
    helper = xorBytes(codeword, bits);
    key = sha256Hex
      ? sha256Hex(
          Array.prototype.map
            .call(hashedBytes, function (b) {
    ...
    return {
      helper: helper,
      params: { rep: rep, version: version, secretBits: secretBitLen },
      key: key,
    };
  }
  ...
  function decode(helper, bits, opts) {
    ...
    rep = opts.rep || 15;
    if (!helper || !bits)
      throw new Error("Helper data and biometric bits are required.");
    if (helper.length !== bits.length)
      throw new Error("Helper and biometric bit lengths differ.");
    totalBits = bits.length * 8;
    secretBitLen = Math.floor(totalBits / rep);
    if (secretBitLen < 8)
      throw new Error("Not enough bits for repetition factor " + rep + ".");
    ...
    for (j = 0; j < secretBitLen; j++)
      setBit(secretBytes, j, getBit(secretPacked, j));
    key = sha256Hex
      ? sha256Hex(
          Array.prototype.map
            .call(secretBytes, function (b) {
              return String.fromCharCode(b);
            })
            .join(""),
        )
